package com.hjwylde.uni.comp103.assignment04.arrayQueue;
...
/**
 * ArrayQueue - a Queue collection; The implementation uses an array to store the items. It
 * "wraps around" the end of the array. When the array is full, it will create a new array of double
 * the current size, and copy all the items over to the new array. It extends AbstractQueue, so it
...
public class ArrayQueue<E> extends AbstractQueue<E> {
    ...
    /**
     * Adds an item onto the back for the queue, unless the item is null If the item was added, it
    ...
    /**
     * Returns the value at the front of the queue, or null if the queue is empty, but does not
     * change the queue
     */
    @Override
    public E peek() {
    ...
    /**
     * Removes and returns the value at the front of the queue, or null if the queue is empty
     */
    @Override
    public E poll() {
        if (isEmpty())
    ...
    private void ensureCapacity() {
        if (((front == 0) && (back == (data.length - 1))) || (front == (back + 1))) { // If
            // queue
            // is
            // full...
    ...
    public static void test() {
        ...
        for (int i = 0; i < (2 * ArrayQueue.INITIAL_CAPACITY); i++) {
            ...
            if (queue.size() != 5)
                System.out.printf("poll and offer failed to maintain queue size \n");
        }
        ...
        for (String str : queue)
            if (!str.equals("v" + (iterpoll++)))
                System.out.printf("iterator did not generate items in expected order at v%d \n",
                        iterpoll - 1);
    ...
    /**
     * Iterator for ArrayQueue. This implementation is not smart, and may be corrupted if any
     * changes are made to the ArrayQueue that it is iterating down. Note that because it is an
     * inner class, it has access to the ArrayQueue's private fields.
     */
    ...
    private class ArrayQueueIterator<E> implements Iterator<E> {
        ...
        /**
         * Remove from the set the last element returned by the iterator. Can only be called once
         * per call to next.
         */
        @Override
        public void remove() {
