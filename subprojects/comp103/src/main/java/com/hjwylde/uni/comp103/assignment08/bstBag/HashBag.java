...
public class HashBag<E> extends AbstractCollection<E> {
    ...
    /**
     * Optional constructor to construct a Bag containing all the elements of the argument
     * collection
     */
    ...
    /**
     * Return an iterator over the elements in this bag. Uses the hashMap iterator, but has to
    ...
    /**
     * Remove an element matching a given item Make no change to the bag and return false if the
     * item is not present.
     */
    ...
    /*
     * A counter class for counting the number of occurrences of an item Like integer, but mutable
     */
    ...
    private class HashBagIterator<T> implements Iterator<T> {
        ...
        private HashBagIterator(HashBag<T> bag, Iterator<Map.Entry<T, Counter>> it) {
            this.bag = bag;
            iter = it;
            getNextReady();
        }
        ...
        /**
         * Get the next item out of the entrySet iterator, ready for the next call to next.
         */
