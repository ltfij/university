package com.hjwylde.uni.comp103.assignment05.forest;
...
public class Dandelion {
    ...
    /**
     * Creates a new Dandelion with random co-ordinates and draws it to the DrawingCanvas
     * <code>c</code>.
     * 
     * @param c the DrawingCanvas to draw the Dandelion on.
     */
    ...
    /**
     * Creates a new Dandelion with the specified x and y co-ordinates. Optionally draws it to the
     * DrawingCanvas.
     * 
     * @param c the DrawingCanvas to draw the Dandelion on.
     * @param x the x co-ordinate.
     * @param y the y co-ordinate.
     * @param draw whether to draw the Dandelion.
     */
    public Dandelion(DrawingCanvas c, double x, double y, boolean draw) {
        ...
        col =
                new Color((float) (0.2 + (0.8 * Math.random())),
    ...
    public void draw(double x, double y, double dist, double ang) {
        ...
        for (double i = 0; i < numBranches; i++)
            draw(newX, newY, shrinkage * dist * (0.3 + rand.nextDouble()),
                    ((ang + (i * branchAngle)) - (branchSpan / 2))
                            + (((0.5 - rand.nextDouble()) * PI) / 6));
    }

    /**
     * Show the growth of this Dandelion over time. It will draw the Dandelion in it's original
     * position and continually be shown to the right of it until the end of the screen.
     */
