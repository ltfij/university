package com.hjwylde.uni.comp103.assignment02.wordCloud;
...
/*
 * Henry Wylde: For the Challenge section: Verticle Dimension Use: I have used the verticle axis to
 * help people find words more easily that they're looking for to compare. For this, I have listed
 * them alphabetically, from a to z, top to bottom. The y position is evenly spread out based on the
 * letter/number combination (0 to 25, for a to z). The font size varies between 10 and 30, rather
 * than 10 and 60, as with a 660 (maximized on a 768 height resolution screen) height canvas the
 * line heights will be about 25 (checked using FontMetrics), so this should keep the a to z lines
 * spread out enough to not overlap each other enough to make it hard to read. Colour Use: I thought
 * it would be very interesting to colour the words by the score that they would get in Scrabble.
 * For this, there is a HashMap with the values of each letter in it and a method used to calculate
 * the value of a word (not including 50pts for more than 7 letters). After calculating the value of
 * a word, it is passed to a method that determines the colour for that word, the colour coding goes
 * from red -> orange -> yellow -> green -> magenta -> cyan -> blue in order of highest to lowest
 * score. The highest word score for the text files given in this assignment was about 36.
 */
...
/**
 * This program reads 2 text files and compiles word counts for each. It then eliminates rare words,
 * and words that only occur in one document, and displays the remainder as a "word cloud" on a
 * canvas, to allow the user to examine differences between the word usage in the two documents.
 */
public class WordCloud implements ActionListener {
    ...
    /**
     * Constructs a WordCloud object, sets up the graphical user interface, and calls the basic
     * method.
     */
    public WordCloud() {
        setUpGUI();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (name.equals("Remove infrequent words")) {
            messageArea.setText("Keeping only the most common " + numWordsToRemove + " words");
    ...
    /**
     * Read the contents of a file, counting how often each word occurs. Put the counts (as Doubles)
    ...
    /**
     * Display words that exist in both documents. The y-position is essentially random (it just
     * depends on the order in which an iterator goes through a Set). However the y-position
     * reflects how much the word is used in the 1st document versus the 2nd. That is, a word that
     * is common in the 1st and uncommon in the second should appear at the top. The SIZE of the
     * word as displayed reflects how common the word is overall, including its count over BOTH
     * documents.
     */
    public void displayWords() {
        ...
        for (String word : allWords) {
            ...
            canvas.setFontSize((int) ((((count1 + count2) * 20) / largestCount) + 10));
    ...
    public double findLargestCount(Set<String> allWords) {
        ...
        for (String word : allWords)
            if (counts1.containsKey(word) && counts2.containsKey(word))
                largestCount = Math.max(largestCount, counts1.get(word) + counts2.get(word));
    ...
    /**
     * Take a Map from words to counts, and "normalise" the counts, so that they are fractions of
     * the total: they should sum to one.
     */
    ...
    /**
     * Print the words and their counts to standard out. Not necessary to the program, but might be
    ...
    /**
     * Takes a Map from strings to integers, and an integer, limitNumWords. It should leave this Map
    ...
    public void setUpGUI() {
        ...
        // The buttons
        JButton readWordsToremoveButton = new JButton("Remove standard common words");
        readWordsToremoveButton.addActionListener(this);
        buttonPanel.add(readWordsToremoveButton);

        JButton readremoveInfrequentWordsButton = new JButton("Remove infrequent words");
        readremoveInfrequentWordsButton.addActionListener(this);
        buttonPanel.add(readremoveInfrequentWordsButton);

        JButton readremoveUnsharedWordsButton = new JButton("Remove un-shared words");
