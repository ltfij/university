...
/**
 * A Centipede is a sequence of segments, represented as a Linked list. Centipede is a specialised
 * form of the LinkedNode class, so that a Centipede object contains a Segment and a reference to
 * the next node. We can view a Centipede as just a single node in a list of nodes, (in which case
 * it could have been called "CentipedeNode") or as the whole list of nodes, represented by its
 * front node. A centipede can: - be created: - grow, adding another segment at the end, - shrink,
 * losing the segment just behing the head, - move forward, with each segment following the one
 * ahead of it, - jump, moving all the segments by the same distance, - split itself in half,
 * returning the new centipede, - reverse its direction, returning the new head, which was the tail
 */
...
public class Centipede implements Comparable<Centipede> {
    ...
    /**
     * Constructor for a new centipede (viewed as a whole list of segments) . A new centipede should
     * be just two segments long, be in a random position, have a random direction, have a random
     * color (which changes along the segments - each segment is a different hue from the previous
     * one)
     */
    ...
    /**
     * Constructor for an element of a centipede. This node should be attached to a previous
     * element, and the position, colour, and direction of segment in this node should depend on the
     * segment of the segment in the previous node: - position is one radius over from the x and y
     * of the previous segment (in the opposite direction from the prevDir), - moving in the same
     * direction as the previous segment - color is the next shade through the spectrum from the
     * previous segment
     */
    ...
    /**
     * move: the centipede should move along, with each segment following the one in front of it.
     * That means that each segment must move in the direction the previous segment moved the last
     * time. The version of move with no arguments will work out a new direction to move (usually,
     * but not always, the same as the current direction of the head) then call move(newDirection).
     * It will turn if it is over the edge, or with a 1 in 10 probability. The version of move with
     * a new direction should first tell the segment after it to move in this segment's current
     * direction, then it should move itself in the new direction, and remember this new direction.
     * Note, move doesn't cause it to redraw - it just changes the current position.
     */
    ...
    /*
     * Jump will move the whole centipede over by (dx, dy) Should jump the first segement, then jump
     * the rest.
     */
    ...
    /*
     * Grow will add a new segment onto the end of the centipede It should have the appropriate
     * position and direction.
     */

    /**
     * redraw should first redraw the rest of the centipede, then it should draw this segment (since
     * the segments nearer the face should be drawn in front of segments further from the face).
     * Each segment knows how to draw itself (as a colored circle, centered at its position, and
     * draw two eyes just above its center if it is a face).
     */
    ...
    /**
     * Reverse will reverse the centipede, turning the head into the tail and the tail into the
     * head, reversing all the links along the way, and returning a reference to the new head Note,
     * it is also necessary to change the direction of travel in each segment so that the centipede
     * will now move exactly the opposite. This is TRICKY!
     */
    ...
    /*
     * As long as there are at least two segments, shorten should remove the very last segment from
    ...
    /**
     * As long as there are at least 3 segments in the caterpiller, shrink will make the centipede
     * shorter by removing the second segment. It has to make all the rest of the centipede move one
     * step to close up the gap, in the direction of the second segment (the one being removed).
     */
    public void shrink() {
        if (size() >= 3) {
    ...
    /*
     * As long as there are at least 4 segments in the centipede, split will break the centipede in
     * half, and return the head of the second centipede (the one starting half way down the
     * existing centipede)
     */
