...
/**
 * Simulation of a collection of centipedes, squirming around the screen looking for food sources.
 * If they find a food source, they eat it. Different kinds of food will have different effects.
 * 
 * This program is related to some of the earlier computer games (now appearing again on a cellphone
 * near you), but doesn't have a game element. (Of course, you could add one.)
 * 
 * The centre of the program is the run() method that loops forever, controlling the simulation. On
 * each cycle ("clock tick") it - moves the centipedes, - makes them eat if at a food source, -
 * redraws them, and then - waits for the next clock tick.
 * 
 * Several of the buttons (split, grow, shrink, reverse) are just for testing your code - they will
 * perform the operation on the first centipede, regardless of the food.
 * 
 * Note: Because the run() method is called from the main method, it is in a different thread from
 * the buttons. That is why the buttons still work, even while the simulation is running.
 */
...
public class Simulation implements ActionListener {
    ...
    /**
     * For each centipede and each food source, check if the centipede is at the food source. If so,
     * perform the appropriate action on the centipede, depending on the type of the food source.
     */
    public void eat() {
        ...
        for (Centipede c : centipedes)
            ...
            for (Integer i : foodSourcesTree.getAllWithin(new Rectangle(c.getX(), c.getY(), c
    ...
    /**
     * Find two centipedes near each other. If one is larger than the other, then remove the smaller
     * one from the set of centipedes.
     */
    public void fight() {
        /*
         * The reason it would be simpler to kill off one centipede at a time is because when
         * iterating through all of the centipedes, if you've added one to the "toRemove" array,
         * then technically it shouldn't be allowed to "fight" any other centipedes near it, but it
         * will unless this is accounted for. The option when not accounting for this is to just
         * remove one centipede at a time and continually check fight() until no more are fighting,
         * and all the centipedes that got eaten have been removed. In my code, I have included a
         * check for "if (!toRemove.contains(centipede))", so if a centipede has lost a fight, it is
         * unable to fight any other centipedes or be fought against, so this is not a problem.
         */
        Set<Centipede> toRemove = new HashSet<>();
    ...
    /**
     * Create a new Set of food items at random places and a new Set with one centipede, and display
     * them.
     */

    public void reset() {
        int num = Integer.parseInt(JOptionPane.showInputDialog("Number of Food Sources:"));
        reset(num);
    }

    public void reset(int numFoodSources) {
        int q =
                Math.max(Simulation.eastEdge - Simulation.westEdge, Simulation.southEdge
                        - Simulation.northEdge); // Select the
        // largest out
        // of width
        // and height.
        ...
        foodSourcesTree =
                new Quadtree(new Rectangle(Simulation.westEdge, Simulation.northEdge, q, q), 9);
        centipedes = new HashSet<>();
        for (int i = 0; i < numFoodSources; i++) {
            FoodSource fs = new FoodSource();
            Rectangle fsBounds = new Rectangle(fs.getX(), fs.getY(), fs.getSize(), fs.getSize());
    ...
    /**
     * A never ending simulation loop that waits for the next tick of the simulation, then - moves
     * each centipede, possibly changing its direction - grows each food source - checks each
     * centipede to see if it is next to some food if so, makes it eat the food, possibly adding a
     * new centipede to the bag of centipedes.
     */
    public void run() {
        while (keepRunning) {
