 ...
 * 
 * It does not allow null as an element of the array. When full it will create a new array of double
...
public class SortingHat<E> {
    ...
    /**
     * Constructs SortingHat out of the given collection that will use the default comparator. It
     * will copy all the items from c into the data array.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Constructs SortingHat out of the given collection that will use the given comparator. It will
    ...
    /**
     * merge method merges from[low..mid-1] with from[mid..high-1] into to[low..high-1] Prints data
     * array after merge using printData
     */
    public void merge(E[] from, E[] to, int low, int mid, int high, Comparator<E> comp) {
        int index = low;
        int indexLeft = low;
        int indexRight = mid;
     ...
     */
    public void mergeSort(E[] data, E[] temp, int low, int high, Comparator<E> comp) {
        if (high > (low + 1)) {
            int mid = (low + high) / 2;
    ...
    /**
     * non-recursive, wrapper method copies data array into a temporary array calls recursive
     * mergeSort method
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * prints the temp array into the screen adds extra spaces to all the words so they are all
     * equal in length after adding the spaces simply returns if length of the temp array is longer
     * than 20 without printing
     */
    ...
    /**
     * sorts data using MergeSort prints time taken by MergeSort prints number of times Merge is
     * called
     */
    public void testMergeSort(SortingHat<E> s) {
        ...
        System.out.println("MergeSort takes: " + (end - start) + " milliseconds");
        System.out.println("Merge sort was called: " + countMergeSort + " times");
    }

    /**
     * sorts data using QuickSort Prints time taken by Quick sort prints number of times partition
     * gets called
     */
    public void testQuickSort(SortingHat<E> s) {
        ...
        System.out.println("QuickSort takes: " + (end - start) + " milliseconds\n");
        System.out.println("Quick sort was called: " + countQuickSort + " times");
    }
    ...
    public void writeFile(E[] temp, String fName) {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(sortedFile.getAbsolutePath()))) {
    ...
    /**
     * Partition into small items (low..mid-1) and large items (mid..high-1) prints data array after
     * partition
     */
    private int partition(E[] data, int low, int high, Comparator<E> comp) {
        int left = low - 1;
        int right = high;
        E pivot = median(data[low], data[high - 1], data[(low + high) / 2], comp);
    ...
    public static void main(String[] args) {
        ...
        List<String> list =
                Arrays.asList("Harry", "Ron", "Hermione", "Dumbledore", "Snape", "Voldemort",
                        "Hagrid", "Remus", "Ginny", "Pettigrew", "Sirius", "Draco");
        ...
        SortingHat<String> ss = new SortingHat<>(list, new ReverseStringComparator());
        ss.testMergeSort(ss);
        System.out.println("=================END OF MERGE SORT - REVERSE==================\n");

        SortingHat<String> s1 = new SortingHat<>(list);
        s1.testQuickSort(s1);
        System.out.println("================END OF QUICK SORT===================\n");

        SortingHat<String> s2 = new SortingHat<>(list, new ReverseStringComparator());
        s2.testQuickSort(s2);
        System.out.println("================END OF QUICK SORT - REVERSE===================\n");

        int print = list.size();

        System.out.println("\n\nThe SORTING HAT says: \n" + s.printSelect(s.data, 0, print / 4)
