...
/**
 * Measures the performance of different implementations of Bag by adding a
 * large number of Strings
 * to a Bag. The strings to be added are read from a file and may or may not
 * contain duplicates.
 */
public class MeasureBags implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals("Read data"))
            ...
            int num = Integer.parseInt(JOptionPane.showInputDialog(frame,
                "Number of items"));
            constructStrings(num,
                (int) Math.ceil((Math.log(num) / Math.log(7)) - 1));
    ...
    /**
     * Construct a List of data strings, The List will have n random strings, each
     * with stringLength
    ...
    public void constructStrings(int n, int stringLength) {
        ...
        textArea.setText("Constructed " + data.size()
    ...
    public void measure(Collection<String> bag) {
        ...
        // report results
        textArea.append("Data has " + data.size() + " items, " + distinct
            + " distinct\n");
        textArea.append("Adding:   " + (afterAdd - start) + " milliseconds\n");
        textArea.append("Contains: " + (afterContains - afterAdd)
            + " milliseconds\n");
        textArea.append("Remove:   " + (afterRemove - afterContains)
            + " milliseconds\n\n");
        textArea.append("Total:    " + (afterRemove - start)
            + " milliseconds\n");
    }
    
    /**
     * Read strings from a file into a List. This ensures that the timing is based
     * on the set
    ...
    public void readStrings() {
        ...
        if (fname == null)
            ...
            try (Scanner sc = new Scanner(new File(fname))) {
                ...
                textArea.setText("Read " + data.size() + " strings from "
                    + fname);
