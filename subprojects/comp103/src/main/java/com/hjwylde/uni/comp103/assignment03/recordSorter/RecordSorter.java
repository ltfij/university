...
public class RecordSorter implements ActionListener {
    ...
    public RecordSorter() {
        ...
        frame.getContentPane().add(new JScrollPane(textArea), BorderLayout.CENTER); // puts
                                                                                    // the
                                                                                    // text
                                                                                    // area
                                                                                    // into
    ...
    private class alphabeticalLastLineComparator implements Comparator<Record> {
        ...
        public int compare(Record record1, Record record2) {
            return record1.getTextline(record1.getNumTextlines() - 1).compareToIgnoreCase(
    ...
    private class averageWordLengthComparator implements Comparator<Record> {
        ...
        public int compare(Record record1, Record record2) {
            ...
            for (int i = 0; i < record1.getNumTextlines(); i++) {
                StringTokenizer line = new StringTokenizer(record1.getTextline(i));
            ...
            for (int i = 0; i < record2.getNumTextlines(); i++) {
                StringTokenizer line = new StringTokenizer(record2.getTextline(i));
    ...
    /**
     * Comparator that will order Records based on the length of their first line of text.
     */
    private class textLineLengthComparator implements Comparator<Record> {

        @Override
        public int compare(Record record1, Record record2) {
            return record1.getTextline(0).length() - record2.getTextline(0).length();
        }
    }

    /**
     * Comparator that will order Records based on the proportion of uppercase letters in the record
     * as a whole.
     */
