 ...
 * 
 * The implementation uses a linked list to store the items. It does not keep
 * the items in any
 * particular order. It does not allow null items. Only requires that the
...
public class LinkedListBag<E> extends AbstractCollection<E> {
    ...
    /**
     * Remove an element matching a given item Return true if the item was present
     * and then removed.
    ...
    public static void test() {
         ...
         * ArrayList<Integer>();
         * Random r = new Random(); for (int i = 0; i < 1000; i++)
         * items.add(r.nextInt(10000));
         * Collection<Integer> numbag = new LinkedListBag<Integer>(); for (int i :
         * items) numbag.add(i);
         * if (numbag.size() != items.size())
         * System.out.printf("Bag should contain %d number \n",
         * items.size()); for (int i : items) { if (!numbag.contains(i))
         * System.out.printf("Bag is missing %d \n", i); } int count = 0; for (int i
         * : numbag) {
         * count++; if (!items.contains(i))
    ...
    private class LinkedListBagIterator implements Iterator<E> {
        ...
        /**
         * Remove from the bag the last element returned by the iterator. Not
         * implemented.
         */
