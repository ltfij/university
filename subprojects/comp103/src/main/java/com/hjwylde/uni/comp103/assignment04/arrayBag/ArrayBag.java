 ...
 * 
 * The implementation uses an array to store the items. It does not keep the items in any particular
 * order, and may change the order of the remaining items when removing items. It does not allow
 * null items. Only requires that the elements can be compared with equals(Object) It is not
 * particularly efficient When full, it will create a new array of double the current size, and copy
...
public class ArrayBag<E> extends AbstractCollection<E> {
    ...
    /**
     * Adds the specified element to this bag. Will not add the null value. Returns true if the
    ...
    /**
     * Remove an element matching a given item Returns true if the item was present and then
     * removed. Makes no change to the bag and returns false if the item is not present.
     */
    @Override
    ...
    /**
     * Find the index of an element in the dataarray, or -1 if not present Assumes that the item is
     * not null
     */
