...
/**
 * Implementation of the Bag type using a binary search tree to store the items. Uses a comparator
 * to compare items. There is a default comparator that works if the items are Comparable Does not
...
public class BSTBag<E> extends AbstractCollection<E> {
     ...
     * collection changes, and false if it does not change.
     */
    @Override
    ...
    /**
     * Remove the element matching a given item Return true if the collection changes, and false if
     * it did not change.
     */
    ...
    public static void Test1() {
        ...
        String[] data =
                {"v21", "v13", "v18", "v16", "v19", "v17", "v12", "v20", "v11", "v26", "v28",
                        "v27", "v15", "v24", "v22", "v23", "v25", "v14", "v29", "v30"};
        ...
        if (bag.remove("v10"))
            System.out.format("Bag should not be able to remove non existant item v10\n");
    ...
    public static void Test2() {
        ...
        System.out
                .println(" Size: " + bag.size() + ", isEmpty: " + bag.isEmpty() + ", Bag: " + bag);

        System.out.print("\nChecking add: ");
        List<String> data =
                Arrays.asList("A", "P", "F", "R", "E", "W", "Q", "T", "Z", "X", "C", "V", "B", "G",
                        "I", "H", "M", "S", "K", "U", "N");
    ...
    /**
     * Iterator for BST To iterate through the tree in sorted order, it must do an iterative
     * in-order traversal, using a stack to keep track of its position To accomplish this, it must
     * process all the left subtree of a node before it processes the node. Therefore, every time it
     * pushes a node on the stack, it must also immediately push all the left descendents of the
     * node on to the stack, all the way to the leftmost descendent. When it pops a node from the
     * stack, it processes the node and then pushes its right child onto the stack (and all that
