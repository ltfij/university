...
/*
 * Code for Assignment 7, COMP 103 Name: Henry J. Wylde Usercode: wyldehenr ID: 300224283
 */
...
public class OrganisationChart implements ActionListener, MouseListener {
    ...
    // Button names
    private final String[][] labels = { {"New", "Load", "Save", "Quit"}, {"Add", "Remove", "Move"},
            {"Employ", "Fire", "Promote", "Reassign"}, {"All Below", "All Above"},
            {"Set Job", "Remove Job"}};
    ...
    // Save / Load
    public static final javax.swing.filechooser.FileFilter CSV_FILTER =
    ...
    /**
     * Perform the current action (stored in currentAction) operates on nodes in pressedNode and
     * releasedNode
     */
    ...
    /**
     * (challenge) Construct a new tree loaded from a file that was saved by doSave.
     */
    private void doLoad() {
    ...
    /**
     * (challenge) Save the whole tree in a file in a format that it can be loaded back in and
     * reconstructed
     */
    private void doSave() {
        JFileChooser fc = new JFileChooser();
            ...
            int choice =
                    JOptionPane.showConfirmDialog(null,
                            "Error with saving file: " + saveFile.getPath()
                                    + ".\nFile already exists.\nOverwrite?", "File Exists Error",
                            JOptionPane.YES_NO_OPTION); // ...then query the user for
                                                        // overwriting.
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(saveFile))) {
            /*
             * CSV File Format: nodeID, nodeEmployeeName, nodeJobTitle, parentID parentID is used to
             * recreate parent links.
             */
    ...
    /**
     * Finds the node in the tree under the given location. Traverses the tree, looking at each
     * node. If the distance between location and the position of the node is less than two times
     * the node radius, then return the node. If no node, then return null
     */
    ...
    /**
     * A node's width is an integer representing the max number of child nodes spanning across the
     * x-axis.
     */
    ...
    /**
     * (Extension) Return true if the node is in the subtree, and false otherwise Uses == to
     * determine node equality since we want to know if this exact node is in the subtree, not just
     * if a node containing the same employee is in the subtree. Only needed for extension version
     * of moveNode
     */
    ...
    /**
     * List (in the text area) the names of all the employees in the reporting chain from this node
    ...
    /**
     * List (in the text area) the names of all the employees under the manager in this node
    ...
    /**
     * Move node to be a subordinate of the "to" node, along with the subtree under node. Note that
     * this is a problem if the "to" node is in the subtree rooted at node (including if the "to"
     * node is the same as node) Testing for this situation is an extension.
     */
    ...
    /**
     * Redraw the chart. First step is to calculate all the locations of the nodes in the tree Then
     * traverse the tree to draw all the nodes and lines between parents and children
     */
    ...
    private void redrawNode(OrgTreeNode nd) {
        ...
        canvas.fillOval(x - nodeRad, y - nodeRad, (nodeRad * 2) - 1, (nodeRad * 2) - 1);
        canvas.setColor(Color.BLACK);
        canvas.drawOval(x - nodeRad, y - nodeRad, (nodeRad * 2) - 1, (nodeRad * 2) - 1);
        ...
        if (job != null)
            canvas.drawString(job.getTitle(), x - (fm.stringWidth(job.getTitle()) / 2), y
                    - (nodeRad + 6));
    }
    ...
    /**
     * Recursive method to draw all nodes in a subtree. The provided code just draws the root node;
     * you need to make it draw all the nodes
     */
    private void redrawNodes(OrgTreeNode nd) {
        for (OrgTreeNode child : nd.getChildren()) {
            canvas.setColor(Color.BLACK);
            canvas.drawLine(nd.getLocation().getX(), nd.getLocation().getY(), child.getLocation()
                    .getX(), child.getLocation().getY());
    ...
    private static boolean employ(OrgTreeNode node) {
        String employeeName =
                JOptionPane.showInputDialog(null, "Enter the employee's name (No coma's):",
                        "Set Employee");
        if (employeeName != null) {
            if (!employeeName.equals("") && !employeeName.contains(",")) // I don't
                                                                         ...
                                                                         // loading
                                                                         // easy.
                return OrganisationChart.employ(node, new Employee(employeeName));

            JOptionPane.showMessageDialog(null, "Error: Illegal employee name entered.",
    ...
    /**
     * Move the employee in this node up to the immediate manager postion, as long as there is
    ...
    /**
     * Move the employee in "node" to the "to" position in the tree, as long as there is currently
     * no employee in that position.
     */
    ...
    /**
     * Remove node and move all its children to be children of its parent Do nothing if the node is
    ...
    private static void setJob(OrgTreeNode node) {
        String jobTitle =
                JOptionPane.showInputDialog(null, "Enter the job's title (No coma's):", "Set Job"); // To
                                                                                                    // allow
                                                                                                    // for
                                                                                                    // easy
                                                                                                    // save
        ...
        if (jobTitle != null)
            if (!jobTitle.equals("") && !jobTitle.contains(","))
                ...
                JOptionPane.showConfirmDialog(null, "Error: Illegal job title entered.",
                        "Error: Job Title", JOptionPane.OK_OPTION, JOptionPane.ERROR_MESSAGE);
    }
