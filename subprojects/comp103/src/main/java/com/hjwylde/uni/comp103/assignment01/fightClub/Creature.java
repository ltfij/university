...
public class Creature {
    ...
    /**
     * Construct a new Creature object. IF dad is not null, based the newbie on his dad.
     */
    public Creature(int arenaWidth, int arenaHeight, DrawingCanvas c, Creature dad, int index,
            Random rng) {
        this.rng = rng;
    ...
    public double calcForce(Creature other, double[] h, double D) {
        ...
        if (D < Creature.bodySize)
            force = (-(Creature.bodySize - D) / Creature.bodySize) * Creature.BodyRepulsionForce;
        return force;
    }
    ...
    /**
     * Add up 'forces' to determine movements, and decide who, if anybody, to attack. But don't DO
    ...
    public String description() { // more verbose than toString...
        ...
        for (String trait : traitnamesIndices.keySet())
            str += String.format("%s\t", trait.substring(0, Math.min(8, trait.length())));
        str += String.format("\nValue: \t");
        for (String trait : traitnamesIndices.keySet()) {
    ...
    public void display(DrawingCanvas canvas, int xpos, int ypos) {
        ...
        canvas.fillOval(xpos - (Creature.bodySize / 2), ypos - (Creature.bodySize / 2),
                Creature.bodySize, Creature.bodySize, false);
        ...
        canvas.fillOval(xpos - (rSize / 2), ypos - (rSize / 2), rSize, rSize, false);

        canvas.setForeground(Color.BLACK);
        // circle size proportional to ability
        int aSize = (int) (Creature.bodySize * Math.sqrt(traitVals[a] / Creature.MAX_ABILITY));
        canvas.drawOval(xpos - (aSize / 2), ypos - (aSize / 2), aSize, aSize, false);

    }

    /**
     * Draw the creature, as a coloured circle, and indicate its victim with a line.
     */
    public void draw() {
        ...
        if (victim != null) {
            ...
            canvas.setForeground(Color.BLACK);
            canvas.drawLine((int) x, (int) y, (int) halfwayX, (int) halfwayY, false);
            canvas.fillOval((int) halfwayX - 3, (int) halfwayY - 3, 6, 6, false);
        }
        ...
        if (rSize > Creature.bodySize) { // Draw resource first
            ...
            canvas.setForeground(Color.LIGHT_GRAY);
            canvas.fillOval((int) x - (rSize / 2), (int) y - (rSize / 2), rSize, rSize, false);
            // draw creature as a coloured disk.
            canvas.setForeground(naturalColour);
            canvas.fillOval((int) x - (Creature.bodySize / 2), (int) y - (Creature.bodySize / 2),
                    Creature.bodySize, Creature.bodySize, false);
        ...
        int aSize = (int) (Creature.bodySize * Math.sqrt(traitVals[a] / Creature.MAX_ABILITY));
        canvas.setForeground(Color.BLACK);
        canvas.drawOval((int) x - (aSize / 2), (int) y - (aSize / 2), aSize, aSize, false);
    }
    ...
    public void move() {
        /**
         * update velocity in response to Force impulse, and "INERTIA" (NB. this isn't really
         * INERTIA though, it's "speed fading"). Assumes deltaX and deltaY have been precomputed
         * elsewhere.
         */
    ...
    public String updateAbilitiesAndResources() {
        ...
        if (!listOfAttackers.isEmpty()) { // Ack! I'm being assaulted!
            double hitpoints =
                    Creature.HITPOINTS_PER_ATTACKER * Math.pow(listOfAttackers.size(), 1.0);
