...
/**
 * A Segment is one part of the body of a centipede. A segment knows - its
 * position on the screen, -
 * its color - the direction it will move next - whether it should have a face
 * on it
 */
...
public class Segment {
    ...
    /**
     * Constructor for the head of a new centipede. A new centipede should be just
     * two segments long,
     * be in a random position, have a random direction, have a random color
     * (which fades along the
     * segments - each segment is a bit more grey than the previous one)
     */
    public Segment() {
    ...
    /**
     * Constructor for a non-head segment. - position is one radius over from the
     * x and y of the
    ...
    /**
     * Returns the new direction that the segment should move in. bounces back
     * from edges, and
     * otherwise turns left or right with a probability of 0.1, turn to the right
     * or left.
     */
    ...
    public void redraw(DrawingCanvas canvas) {
        ...
        canvas.fillOval(x - Segment.size, y - Segment.size, Segment.size * 2,
            Segment.size * 2, false);
        
        canvas.setForeground(Color.black);
        canvas.drawOval(x - Segment.size, y - Segment.size, Segment.size * 2,
            Segment.size * 2, false);
        
        if (face) {
            canvas.setForeground(Color.black);
            canvas.fillOval(x - (Segment.size / 2), y - (Segment.size / 2),
                Segment.size / 3, Segment.size / 3, false);
            canvas.fillOval(x + (Segment.size / 6), y - (Segment.size / 2),
                Segment.size / 3, Segment.size / 3, false);
        }
    }
