...
/** Read a file and check that the tags match and nest properly
    ...
    */
    public void checkTags() {
        /*
         * Completion: The reason that when one tag causes an error that many consequential tags
         * will report errors too is that once the error is found, the "tag pair" (opening and
         * closing tag) are popped off of the stack. This means that for tags that are spelling
         * mistakes, it won't cause consequential tag errors, however for missing closing tags or
         * missing opening tags, this will result in a lot of further errors as the heirarchy of
         * tags will have been skewed by one. There are a few different basic types of tag errors
         * that could occur: 1. Missing opening or closing tag. (Solved by not popping the "error"
         * tag off the stack.) Missing closing tags at the end are solved by doing a check for any
         * left-over items in the stack. 2. Misspelt or incorrect opening or closing tag. (Solved by
         * popping the "error" tag off the stack.) 3. A closing tag in the wrong position. (Similar
         * to a missing tag, and shall be treated as one as it is dificult to search through a stack
         * for a specific tag.) Challenge: To solve these problems, the program has been designed to
         * pop the tags off of the stack when an incorrect one has been detected. For the missing
         * tag cases, I have just done a check whether the received tag is the next expected tag
         * (the next tag on the tagStack), and if it is, it will pop off the next expected tag as
         * well and report a missing tag. It can't go back more than one tag to check however. This
         * is by no means a 'fool proof' way of solving the problem of determining incorrect tags,
         * however it is an adequate one for basic problems. In reality, it would be next to
         * impossible to correctly determine incorrect tags as it is hard to determine exactly what
         * the human was thinking/meaning when they wrote an incorrect tag. To properly solve it,
         * the program should iterate through the file, and after each incorrect tag ask the user to
         * fix it, so it may continue checking the rest of the tags without any previous errors.
         */
        ...
        try (Scanner s = new Scanner(new File(fileName))) {
                        ...
                        if (tagStack.peek().equalsIgnoreCase(token)) // If the closing tag
                            ...
                            if (!tagStack.isEmpty()) {
                                ...
                                if (token.equalsIgnoreCase(nextExpectedTag)) {
                                    ...
                                    // partner.
                                    textArea.append("Tag Error:\nMissing Tag: </" + expectedTag
                                            + ">\n");
                                    tagStack.pop();
                                } else if (textArea
                                        .getText()
                                        .substring(
                                                textArea.getText().length() - (17 + token.length()))
                                        .equalsIgnoreCase("Missing Tag: </" + token + ">\n")) {
            ...
            while (!tagStack.isEmpty())
                ...
                textArea.append("Tag Error:\nLeft Over Tag was not Closed: <" + tagStack.pop()
                        + ">\n");
        } catch (IOException e) {
        }
    }
