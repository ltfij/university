...
/**
 * Create a random job file for testing CarlsCourier simulation. Each line in the file contains a
 * description of a courier delivery job, giving the time when the order is received, the charge
 * rate (P[remium], S[tandard] or B[udget]), and the destination suburb. Possible destination
 * suburbs are read from the file places.txt (this also has distance information which is used in
 * the simulation but is not needed here).
 * 
 * The program steps through the minutes in a day (from 9am to 4pm), choosing whether to generate an
 * order and if so which kind of order. Times are in minutes from 9am, so range from 1 to 420.
 * Budget jobs are not accepted after noon, nor are standard jobs after 2pm or premium jobs after
 * 4pm. The program will not generate jobs that won't be accepted, but you can suppress that test
 * and check that your program will reject them.
 * 
 * The program can take up to two command line arguments. The first argument gives the stem for the
 * names of the files (default is 'test"), and the second specifies the number of files to be
 * created (default is 1). If one argument is given, it is taken as the number of files if it is a
 * number, and as the stem otherwise. Thus, if no argument is given, the program creates a file
 * called test1.
 */
...
public class MakeJobFile {
    ...
    private static final double[] probability = {0.15, 0.25, 6};
    ...
    public static void main(String[] args) {
        ...
        System.out.println("  stem = " + stem + "number of files = " + numFiles); // for
                                                                                  // testing
    ...
    private static void createFile(String name) {
        ...
        try (PrintStream ps = new PrintStream(new File(name))) {
            for (int time = 1; time <= 520; time++) {
                ...
                String dest =
                        MakeJobFile.places.get(MakeJobFile.rand.nextInt(MakeJobFile.places.size()));
