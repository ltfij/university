...
/*
 * Code for Assignment 6, COMP 103 Name: Henry J. Wylde Usercode: wyldehenr ID: 300224283 Reference
 * for what a Quadtree is: http://www.cs.ubc.ca/~pcarbo/cs251/welcome.html. Example Quadtree applet:
 * http://donar.umiacs.umd.edu/quadtree/regions/regionquad.html. This code I wrote for a quadtree is
 * very basic, and omits many basic features you would normally expect, such as remove(int index) or
 * remove(Rectangle bounds). I only added in the main functionality that it was intended for: adding
 * in FoodSources and searching for them, but as a FoodSource never moves or is never removed, I did
 * not create these methods. (These methods I will add in (when I have more spare time) and when I
 * want to improve it and use it for more functions.) The rough idea of the structure of this
 * Quadtree: Each QuadtreeNode has 4 quadrants, each with a specific bounds that represent it's
 * area. These quadrants can be split down into further quadrants a certain number of times
 * (maxLevels). A node has 1 index, which is an integer representing an index for some object in
 * some array (in this assignment, the index is the FoodSource in the FoodSource array). An index of
 * a node is within an entire node's bounds, so if the index only covers a small bounding area then
 * it is best to make the Quadtree divide up into many small quadrants (as far down as 1x1 is
 * accurate, 2x2 can work for roughness but is not as accurate).
 */
...
public class Quadtree {
     ...
     * 
     * @param root create a <code>Quadtree</code> with the given <code>root</code>
     */
     ...
     * @param maxLevels max number of division levels.
     */
     ...
     * 
     * @param bounds the area to search.
    ...
    /**
     * Inserts the given <code>index</code> in the given <code>bounds</code>. If an index is already
     * within the bounds given, then that index will be overwritten with the new index.
     * 
     * @param index the index.
     * @param bounds the bounds of the index.
     */
    ...
    public class QuadtreeNode {
        ...
        /**
         * Create a new <code>QuadtreeNode</code> with the specified parent node, bounding area and
         * max division levels.
         * 
         * @param parent the nodes parent.
         * @param bounds the nodes bounds.
         * @param maxLevels the nodes max division levels.
         */
        ...
        public void dumpTree(Graphics g) {
            if (index != Quadtree.NODE_TRANSPARENT) {
                ...
                g.drawLine(bounds.x, bounds.y + (bounds.height / 2), bounds.x + bounds.width,
        ...
        /**
         * Gets the max levels this node can be divided into (depth of the Quadtree from this node).
         * 
         * @return the max levels.
         */
        public int getMaxLevels() {
        ...
        /**
         * Inserts the given <code>index</code> in the given <code>bounds</code>. If an index is
         * already within the bounds given, then that index will be overwritten with the new index.
         * 
         * @param index the index.
         * @param bounds the bounds of the index.
         */
        public void insert(int index, Rectangle bounds) {
            if (bounds.equals(this.bounds))
                ...
                if (subNodes == null) {
                    ...
                    subNodes[Quadtree.TOP_RIGHT] =
                            new QuadtreeNode(this, getQuadrantBounds(Quadtree.TOP_RIGHT),
                                    maxLevels - 1);
                    subNodes[Quadtree.TOP_LEFT] =
                            new QuadtreeNode(this, getQuadrantBounds(Quadtree.TOP_LEFT),
                                    maxLevels - 1);
                    subNodes[Quadtree.BOTTOM_LEFT] =
                            new QuadtreeNode(this, getQuadrantBounds(Quadtree.BOTTOM_LEFT),
                                    maxLevels - 1);
                    subNodes[Quadtree.BOTTOM_RIGHT] =
                            new QuadtreeNode(this, getQuadrantBounds(Quadtree.BOTTOM_RIGHT),
                                    maxLevels - 1);
                }

                // Insert the index into applicable quadrants.
                if (bounds.intersects(getQuadrantBounds(Quadtree.TOP_RIGHT)))
                    subNodes[Quadtree.TOP_RIGHT].insert(index,
                            bounds.intersection(getQuadrantBounds(Quadtree.TOP_RIGHT)));
                if (bounds.intersects(getQuadrantBounds(Quadtree.TOP_LEFT)))
                    subNodes[Quadtree.TOP_LEFT].insert(index,
                            bounds.intersection(getQuadrantBounds(Quadtree.TOP_LEFT)));
                if (bounds.intersects(getQuadrantBounds(Quadtree.BOTTOM_LEFT)))
                    subNodes[Quadtree.BOTTOM_LEFT].insert(index,
                            bounds.intersection(getQuadrantBounds(Quadtree.BOTTOM_LEFT)));
                if (bounds.intersects(getQuadrantBounds(Quadtree.BOTTOM_RIGHT)))
                    subNodes[Quadtree.BOTTOM_RIGHT].insert(index,
                            bounds.intersection(getQuadrantBounds(Quadtree.BOTTOM_RIGHT)));
            } else
        ...
        /**
         * Returns true if this node is transparent. Note: At this moment I have not added in
         * functionality for parent nodes transparency to be altered if all subnodes have the same
         ...
         * 
         * @param area the quadrant area (TOP_RIGHT, TOP_LEFT, BOTTOM_LEFT, BOTTOM_RIGHT).
        ...
        private Rectangle getQuadrantBounds(int area) {
            ...
            switch (area) {
                ...
                case (Quadtree.TOP_LEFT):
                    quadBounds =
                            new Rectangle(bounds.x, bounds.y, bounds.width / 2, bounds.height / 2);
                    break;
                case (Quadtree.BOTTOM_LEFT):
                    quadBounds =
                            new Rectangle(bounds.x, bounds.y + (bounds.height / 2),
                                    bounds.width / 2, bounds.height / 2);
                    break;
                case (Quadtree.BOTTOM_RIGHT):
                    ...
                    break;
                default:
                    throw new IllegalArgumentException(area + " is not a valid quadrant area.");
            }
