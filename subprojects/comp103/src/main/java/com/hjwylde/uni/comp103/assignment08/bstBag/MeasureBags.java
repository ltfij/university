...
/**
 * Measure the performance of different Bag implementations by timing a list of bag operations. Can
 * perform two kinds of test on any of the available bag implementations: 1. Generate a random list
 * of strings then measure the time to add them all, test them all, and them remove them all. 2.
 * Read a list of strings from a file, each preceded by +, - or ? indicating whether it is to be
 * added, removed or tested, and measure the time to perform all operations on the seleeted
 * implementation. There is a button to generate random data and one to read data from a file, and a
 * button for each bag implementation, which runs the appropriate experiment (according to which of
...
public class MeasureBags implements ActionListener {
    ...
    private final String[] buttons = {"Random data", "Load file", "Array", "Sorted Array",
            "Linked List", "BST", "Hash", "Quit"};
    ...
    /**
     * Generate a List of n strings, each with stringLength characters, chosen at random from
     * 'a'..'g'. Then make a list of test commands to add all of the strings, test whether each is
     * in the bag, then remove them all.
     */
    ...
    public void generateStrings(int n, int stringLength) {
        ...
        textArea.setText("Constructed " + randomData.size() + " random strings of length "
                + stringLength + "\n");
    }
    ...
    /**
     * Read all of the operations from a test files into data (List). Each line must be either +s to
        ...
        try (Scanner f = new Scanner(new File(fname))) {
            ...
            while (f.hasNext()) {
                ...
                if ((c == '+') || (c == '-') || (c == '?'))
                    ...
                    JOptionPane.showMessageDialog(null, "Invalid string " + str + " ignored");
            }
    ...
    /**
     * Perform an experiment using the selected bag implementation, according to whether the
     * "Random data" or "Load file" button was clicked most recently. Should never have randomData
     * and fileData both non-null.
     */
    ...
    /**
     * Measure the performance of a bag implementation on a given set of data Measure how long it
     * takes and print out average time per element in nanoseconds, and number of adds, removes and
    ...
    /**
     * Perform an experiment using randomly generated data. Measure the time taken to add all of the
     * strings, test all of the strings, and remove all of the strings. Display the time taken for
