...
/*
 * Code for Assignment 7, SWEN 221 Name: Henry J. Wylde Usercode: wyldehenr ID: 300224283
 */

public class ArraySetTests {

    private static final Integer[] RAW_DATA = ArraySetTests.generateRawData(0);
    private static final Integer[] RAW_DATA_2 = ArraySetTests.generateRawData(-1);

    private static final List<Integer> LIST_DATA = Arrays.asList(ArraySetTests.RAW_DATA);
    private static final List<Integer> LIST_DATA_2 = Arrays.asList(ArraySetTests.RAW_DATA_2);
    ...
    public void testSize() {
        Assert.assertEquals(ArraySetTests.genAS().size(), ArraySetTests.RAW_DATA.length);
        Assert.assertEquals(new ArraySet<Integer>().size(), 0);
    }

    @Test
    public void testToArray() {
        Assert.assertTrue(Arrays.equals(ArraySetTests.genAS().toArray(), ArraySetTests.RAW_DATA));
    }

    @Test
    public void testToEArray() {
        Assert.assertTrue(Arrays.equals(ArraySetTests.genAS().toArray(new Integer[0]),
                ArraySetTests.RAW_DATA));
        Assert.assertTrue(Arrays.equals(ArraySetTests.genAS().toArray(new Integer[100]),
                ArraySetTests.RAW_DATA));
        Assert.assertTrue(Arrays.equals(ArraySetTests.genAS().toArray(new Integer[150]),
    ...
    public void testToString() {
        ...
        Assert.assertTrue(ar.equals(new TreeSet<>(ArraySetTests.LIST_DATA).toString()));
    }
    ...
    public void testValidAddAll() {
        ...
        Assert.assertFalse(as.addAll((Collection<Integer>) new ArraySet<Integer>()));
    ...
    public void testValidContains() {
        ...
        for (char c : "Wobble fish like to go swimming down the stream.".toCharArray())
    ...
    public void testValidContainsAll() {
        Assert.assertTrue(ArraySetTests.genAS().containsAll(ArraySetTests.genAS()));
        Assert.assertFalse(new ArraySet<Integer>().containsAll(ArraySetTests.genAS()));
        Assert.assertFalse(ArraySetTests.genAS().containsAll(
                new ArrayList<>(ArraySetTests.LIST_DATA_2)));
        Assert.assertTrue(ArraySetTests.genAS().containsAll(ArraySetTests.LIST_DATA));
        Assert.assertTrue(ArraySetTests.genAS().containsAll(new ArraySet<Integer>()));
