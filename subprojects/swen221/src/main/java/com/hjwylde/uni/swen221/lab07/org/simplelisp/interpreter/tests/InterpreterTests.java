...
public class InterpreterTests {
    ...
    public void testError() {
        try {
            ...
            Assert.assertEquals("incorrect error message", "test message",
                e.getMessage());
        }
    }
    ...
    public void testEvaluate() {
        ...
        try {
            String s;
            s = "(+ 2 (* 0 (+ 1 3)))";
            Assert.assertEquals(new Interpreter().evaluate(Parser.parse(s))
                .toString(), "2");
            s = "(+ 2 (* (- 0 1) (- (- 0 1) 3)))";
            Assert.assertEquals(new Interpreter().evaluate(Parser.parse(s))
                .toString(), "6");
    ...
    public void testType_check() {
        ...
        try {
            new Interpreter().type_check("test", new LispExpr[] {
                new LispString("boo"), new LispInteger(5)
        ...
        try {
            new Interpreter().type_check("test", new LispExpr[] {
                new LispString("boo"), new LispChar('c')
            }, LispString.class, LispChar.class, LispNil.class);
        ...
        try {
            new Interpreter().type_check("test", new LispExpr[] {
                c1, c2
            }, LispChar.class, LispChar.class, LispNil.class);
            Assert.fail("Fail");
        } catch (Error e) {}
    }
}
