...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            int numRandomRobots = Main.inputInteger("How many randomBots?", scanner);
            int numTurfRobots = Main.inputInteger("How many turfBots?", scanner);

            for (int i = 0; i != numRandomRobots; i++)
                battle.addRobot(new RandomBot(new Point(Main.randomInteger(battle.getArenaWidth()),
                        Main.randomInteger(battle.getArenaHeight()))));

            for (int i = 0; i != numTurfRobots; i++)
                battle.addRobot(new TurfBot(new Point(Main.randomInteger(battle.getArenaWidth()),
                        Main.randomInteger(battle.getArenaHeight()))));
