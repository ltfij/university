...
public class test2 extends BasicMatcher {
     ...
     */
    static final String possibleRegexChars =
    ...
    public void extraTest() {
        ...
        Assert.assertTrue(match(
                "pgmykNpZYmnKferQJRVQGPEfKeyIKilctmrdgtjdneVMkOsujnAKHXBBDHCRpQhnkgecUyoOeB", "o"));
        Assert.assertFalse(match("", "fKED.GQKMew"));
        Assert.assertFalse(match("", "K*hcSiVpMNahwwpFdiDc$wk*bQaiRgoFx^$nQlFrwgCjbn^."));
        Assert.assertFalse(match("zEoQlbwIIekwVObCLJLROhKoZXleoqaNbThglPcHfFUxZUiniBAGecYmPH",
                "$*k^"));
        Assert.assertFalse(match(
        ...
        Assert.assertTrue(match(
                "hLTzINJekzmxyLxYOOpufpHNTRaGnFGqUTdydtnZCyRDjxRILrwXTVDRTyGreQTKpJQuY", "$K*"));
        Assert.assertTrue(match(
                "vJDLGVfNnLmQiQfLAgAQznskFhtScHEGNbhJRVLjRzFIOLgriyzXDgZjYjQmOEpSfndhVCSRygXLfEbULiMk",
                "$"));
        Assert.assertFalse(match("SBMXlnFFzkNstahmakCMaZvcbOghTMUWeWZmJTlLZNpiIaqqCXyswTudL", "$u"));
        ...
        Assert.assertFalse(match("RQeOvWcQTbGptxFDmRhGrTmJaaijLecNBalhLpEakdYVwbpWlJtNpwt", "j$L"));
        Assert.assertTrue(match(
        ...
        Assert.assertTrue(match(
                ...
                "W++"));
        Assert.assertTrue(match("ITwmBYHyNHUcfZyCxmnFfOQBpOLZqbeBGXaqFtiwcfNADGnQQFvRhlyk", "S*+"));
        Assert.assertTrue(match("vjDg", "$*"));
        Assert.assertFalse(match(
                "IbcYcImuozdidIQEoIQuMmXdyxoFMGrwCwpNoCofIbjlIFXhzZuvdBvjErOjZMpYePmoc", "X+$"));
        Assert.assertTrue(match("SokAliQdjsKxtDLxypXVdrYrBqp", "$+"));
        Assert.assertFalse(match("YiMocnHsloiQgNtDJITqwkVxrKEDEoWNZERkakUYwMEuRxNeBIMrp", "w*N^"));
    ...
    private void randomTest() {
        ...
        String inputRegex = test2.generateRandomRegex((int) (Math.random() * 5));
        ...
        final String msg =
                "match(\"" + inputText + "\", \"" + inputRegex + "\") should be " + expected;
