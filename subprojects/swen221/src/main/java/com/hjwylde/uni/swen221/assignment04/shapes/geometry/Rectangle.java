...
public class Rectangle implements Shape {
     ...
     * any area - in other words, no <code>(x, y)</code> point is contained inside of the rectangle.
     * 
    ...
    public static Rectangle difference(Rectangle left, Rectangle right) {
        ...
        if ((right.y > left.y) || ((right.y + right.height) < (left.y + left.height)))
        ...
        if ((right.x > left.x) || ((right.x + right.width) < (left.x + left.width)))
        ...
        return new Rectangle(x1, y1, (((x2 - x1) < 0) ? 0 : (x2 - x1)), (((y2 - y1) < 0) ? 0
                : (y2 - y1)));
     ...
     * null if the rectangles don't intersect at all - note that this is different to
