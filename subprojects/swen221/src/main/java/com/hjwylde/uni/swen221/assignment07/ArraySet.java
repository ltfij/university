...
public class ArraySet<T extends Comparable<T>> implements Set<T> {
    ...
    public <E> E[] toArray(E[] array) {
        if (array.length < size)
            ...
            array =
                    (E[]) java.lang.reflect.Array.newInstance(array.getClass().getComponentType(),
                            size);
     ...
     * Returns -1 if the object isn't a compatible type. Note that this only happens in cases where
     * the actual value of the index doesn't matter, only the fact that it's negative.
     * 
    ...
    private static final class InternalIterator<S> implements Iterator<S> {
        ...
        @Override
        public void remove() {
            throw new UnsupportedOperationException("SortedSet.Iterator.remove() is unsupported");
        }
    }
}
