...
public class Board {
     ...
     * a 2D grid of bits, where each bit represents a wall.
     */
     ...
     * state will be broadcast by a master connection, and is then used to overwrite the current
     * state (since it should be more up to date).
     * 
    ...
    public synchronized int registerPacman() {
        ...
        Character r =
                new Pacman(portal[0] * 30, portal[1] * 30, MovingCharacter.STOPPED, ++Board.uid, 3,
                        0);
        characters.add(r);
        return Board.uid;
    }
     ...
     * broadcast by a master connection when the connection is established.
     */
