...
public class Castling implements MultiPieceMove {
     ...
     * are implemented (they create new Piece's / copies) flags won't work.
     */
     ...
     * for the color and one for what side to castle on.
     * 
    ...
    public void apply(Board board) {
        ...
        board.move(kingPos, new Position(kingPos.row(), kingPos.column() + (kingSide ? 2 : -2)));
        board.move(rookPos, new Position(kingPos.row(), kingPos.column() + (kingSide ? 1 : -1)));
    }
    ...
    public boolean isValid(Board board) {
        ...
        if (board.isInCheck(new Position(kingPos.row(), kingPos.column() + (kingSide ? 1 : -1)),
                isWhite))
            return false;
