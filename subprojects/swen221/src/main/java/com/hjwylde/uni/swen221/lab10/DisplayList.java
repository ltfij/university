...
public class DisplayList<T> extends ArrayList<T> {
     ...
     * thread. The idea is that everytime a thread sets an item in the list, we update it's last
     * touched information. In particular, the touchedMap maps a thread to its last touched index,
     * whilst the reverse touched map maps an index to the thread that last touched it.
     */
    ...
    private static Color[] colors = {Color.RED, Color.BLUE, Color.GREEN, Color.WHITE, Color.GRAY,
            Color.CYAN,};
    ...
    public DisplayList(Collection<T> items) {
        ...
        scroller =
                new JScrollPane(canvas, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
