...
public class Calculator {
     ...
     * expression
     * that is wanted to be evaluated.
     * 
     ...
     * it
     * is unable to parse it, it will throw a RuntimeException. If there is no input then it returns
     * 0. If the expression is evaluating a set call, then this method returns the new value of the
     * variable.
     * 
    ...
    public BigDecimal evaluate() throws RuntimeException {
        ...
        if (lookahead == '(')
            ...
            value = readNumber();
        else if ((input_.length() > (index_ + 3))
            && input_.substring(index_).startsWith("set ")) {
        ...
        if (index_ < input_.length()) {
            ...
            /*
             * FIXME: BEDMAS problem
             * 
             * This problem is rather big... Causes 4 * 2 + 2 to return 16 rather than 10. It can't
             * easily
             * be fixed (to my knowledge) with how the calculator has been implemented so far. It
             * would
             * require a bit of restructuring of all the methods and classes - so for now I have not
             * fixed
             * it.
             */
            if (lookahead == '+') {
    ...
    private void error() throws RuntimeException {
        throw new RuntimeException("Cannot parse character '"
            + input_.charAt(index_) + "' at position " + index_ + " of input '"
            + input_ + "'\n");
    }
    ...
    private String readString() {
        ...
        while ((index_ < input_.length())
            && (Character.isLetter(input_.charAt(index_))))
            index_++;
    ...
    private void skipWhitespace() {
        while ((index_ < input_.length())
            && Character.isWhitespace(input_.charAt(index_)))
            index_++;
    }
