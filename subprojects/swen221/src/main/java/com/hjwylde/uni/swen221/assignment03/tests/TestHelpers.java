package com.hjwylde.uni.swen221.assignment03.tests;
    ...
    public static void checkInvalidTests(String[] tests) {
        for (String input : tests)
            try {
                ...
                if (boards.isEmpty())
                    Assert.fail("test failed with insufficient boards on input: " + input);
                ...
                e.printStackTrace();
                Assert.fail("test failed because of exception on input: " + input);
            }
    }
        ...
        for (String[] test : tests) {
            ...
            try {
                ...
                if (!actualOutput.equals(expectedOutput)) {
                    ...
                    Assert.fail("test failed with incorrect last board on input: " + input);
                }
            } catch (Exception e) {
                System.out.println(e);
                e.printStackTrace();
