...
public final class GameEngine implements Runnable {
    ...
    @Override
    public void run() {
        checkState(!board.isGameOver(), "The cluedo game has already been played on this board!");
     ...
     */
    private Optional<Path<Coordinate>> checkPlayerMove(int player, Coordinate end, int numberMoves,
            int turnStart) {
    ...
    private void initGameState() {
        ...
        Room murderRoom =
                Room.murderRoomValues()[(int) (Math.random() * Room.murderRoomValues().length)];
    ...
    private void playerMakeAccusation(int player) {
        Triple<Character, Weapon, Room> accusation =
                ui.queryPlayerMakeAccusation(board.getPlayer(player));
    ...
    private int playerMove(int player, int turnStart) {
        ...
        // Validate the move
        Optional<Path<Coordinate>> path = checkPlayerMove(player, move, 1000000000, turnStart);
        if (!path.isPresent()) {
    ...
    private int playerMove(int player, int maxMoves, int turnStart) {
        Coordinate move =
                checkNotNull(ui.queryPlayerMove(board.getPlayer(player), maxMoves),
                        "Co-ordinate cannot be null");

        // Validate the move
        Optional<Path<Coordinate>> path = checkPlayerMove(player, move, maxMoves, turnStart);
        if (!path.isPresent()) {
            ui.informPlayerIllegalMove(board.getPlayer(player));
            return 0; // Don't count this move or do anything
        }
        ...
        if (board.getTile(board.getPlayerPosition(player)) == Tile.INTRIGUE_CARD) {
            ...
            ui.informPlayerNumberMovesLeft(board.getPlayer(player), maxMoves - path.get().length()
                    - 1);
    ...
    private void playerPickUpIntrigueCard(int player) {
        ...
        if (card instanceof ClockCard) {
            int remaining =
                    Iterables.size(Iterables.filter(board.getIntrigueCards(), ClockCard.class));
            ...
            if (remaining == 0) {
                ...
                ui.informPlayerPickUpClockCard(board.getPlayer(player), remaining);
        } else { // card instanceof KeeperCard
            ui.informPlayerPickUpKeeperCard(board.getPlayer(player), (KeeperCard) card);

            board.getPlayer(player).addKeeperCard((KeeperCard) card);
        }
    }

    private boolean playerRefuteRumour(int player, int refutePlayer, Set<Card> rumourCards) {
        Set<Card> cards = new HashSet<>(board.getPlayer(refutePlayer).getCards());
        ...
        if (cards.size() == 0) // This player cannot refute the rumour, tell the UI and skip them
            ...
            Card response =
                    ui.queryPlayerChooseRefuteRumourCard(board.getPlayer(refutePlayer), cards);
    ...
    private void playerShowPlayerCard(int playerFrom, int playerTo, Card card) {
        ...
        for (int i : kc.findPlayersWithKeeperCard(ViewRumourRefuteCard.class, playerFrom, playerTo))
    ...
    private void playerStartRumour(int player) {
        Pair<Character, Weapon> rumour = ui.queryPlayerStartRumour(board.getPlayer(player));
    ...
    private void runPlayerTurn(int player) {
        ...
        TURN: while (true) {
            ...
            if (!board.getTile(board.getPlayerPosition(player)).isRoom() || startedRumour)
            ...
            Coordinate turnStartCoord = board.getPlayerHistory(player).get(turnStart);
            ...
            PlayerTurnChoice choice = ui.queryPlayerTurnChoice(board.getPlayer(player), choices);
            switch (choice) {
                case MOVE:
