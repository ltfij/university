...
public final class CLI implements UI {
    ...
    public CLI(Board board, InputStream in, OutputStream out) {
        ...
        this.in = new BufferedReader(new InputStreamReader(Objects.requireNonNull(in)));
    ...
    public void informPlayerClockCardLoss(Player player) {
        out.println();
        out.printf("%s has just picked up the 8th clock card! They are now out of the game.%n",
                player);
    ...
    public void informPlayerMoveAnywhere(Player player) {
        out.println();
        out.printf("%s has played their keeper card and can move anywhere they like this turn!%n",
                player);
    ...
    public void informPlayerMovedToStart(Player player, Character moved) {
        out.println();
        out.printf("%s has just moved %s to their starting position!%n", player, moved);
    ...
    public void informPlayerPickUpClockCard(Player player, int remaining) {
        out.println();
        out.printf("%s has just picked up the %s%s clock card!%n", player, 8 - remaining,
                remaining == 7 ? "st" : remaining == 6 ? "nd" : remaining == 5 ? "rd" : "th");
    ...
    public void informPlayerPickUpKeeperCard(Player player, KeeperCard card) {
        out.println();
        out.printf("%s has just picked up the \"%s\" keeper card!%n", player, card);
    ...
    public void informPlayerViewKeeperCards(Player player) {
        ...
        if (player.getKeeperCards().size() == 0)
            ...
            for (KeeperCard card : player.getKeeperCards()) {
                ...
                int quantity =
                        Iterables.size(Iterables.filter(player.getKeeperCards(), card.getClass()));

                out.printf("%s x%s%n\t%s%n", card.getName(), quantity, card.getDescription());
            }
        }
    ...
    public void informPlayerWin(Player player, Triple<Character, Weapon, Room> accusation) {
        ...
        out.printf("%s was killed with the %s in the %s room.%n", accusation.getFirst(),
                accusation.getSecond(), accusation.getThird());
    ...
    public void informPlayerWrongAccusationLoss(Player player) {
        out.println();
        out.printf("%s has just made a wrong accusation! They are now out of the game.%n", player);
    ...
    public Card queryPlayerChooseRefuteRumourCard(Player player, Set<Card> options) {
        out.println();
        out.printf("%s please refute the rumour with one of your cards:%n", player);
    ...
    public Coordinate queryPlayerMove(Player player, int maxMoves) {
        out.println();
        out.printf("You have %s move%s left to make.%n", maxMoves, maxMoves == 1 ? "" : "s");
        printBoard(player);
    ...
    public Character queryPlayerMovePlayerToStart(Player player, List<Character> options) {
        out.println();
        out.printf("Which player would you like to move to the start?%n", player);
    ...
    public Option queryPlayerPlayKeeperCard(Player player, KeeperCard card) {
        out.println();
        out.printf("%s, would you like to play the \"%s\" keeper card? (%s)%n", player,
                card.getName(), card.getShortDescription());
    ...
    public Card queryPlayerShowCard(Player playerFrom, Player playerTo, Set<Card> options) {
        out.println();
        out.printf("%s,  please select a card to show %s:%n", playerFrom, playerTo);
    ...
    private void printTile(Player player, Coordinate pos) {
        ...
        for (int i = 0; i < board.getNumberPlayers(); i++) {
            if (board.isPlayerDead(i) || !board.getPlayerPosition(i).equals(pos))
                continue;
    ...
    private <T> T query(List<T> options) {
        checkArgument(options.size() > 0, "Must have at least one option for the user!");
        ...
        while (true) {
            ...
            out.printf("Sorry, but \"%s\" is not a valid option. Please select again.%n", answer);
            out.flush();
        }
    }
    ...
    private Coordinate queryCoordinate() {
        ...
        if (!board.isValidCoordinate(coord)) {
            out.printf("Sorry, but \"%s\" is not a valid co-ordinate. Please enter another: ",
                    coord);
            out.flush();
