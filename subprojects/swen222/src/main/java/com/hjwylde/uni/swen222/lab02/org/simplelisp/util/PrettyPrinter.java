...
public class PrettyPrinter {
    ...
    private static String print(LispExpr e, int indent) {
        if ((e instanceof LispInteger) || (e instanceof LispChar) || (e instanceof LispNil)
        ...
        if (head instanceof LispSymbol) {
            ...
            if (fn.name().equals("progn")) {
                ...
                for (int i = 1; i < e.size(); ++i)
                    ret += padNewline(indent + 6) + print(e.get(i), indent + 6);
                return ret + ")";
            }
            ...
            if (fn.name().equals("if"))
                if (e.size() == 4) {
                    ret += "if ";
                    ret += print(e.get(1), indent);
                    ret += padNewline(indent + 6) + print(e.get(2), indent + 6);
                    ret += padNewline(indent + 3) + print(e.get(3), indent + 3);
                    return ret + ")";
                } else if (e.size() == 3) {
                    ret += "if ";
                    ret += print(e.get(1), indent);
