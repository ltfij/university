...
public final class PlayerPanel extends JPanel implements ActionListener {
    ...
    public void update() {
        ...
        jLabelPlayerName.setText("Player: " + board.getPlayer(player).toString());

        List<CharacterCard> characterCards = new ArrayList<>();
        List<String> hovers = new ArrayList<>();
        for (CharacterCard card : filter(board.getPlayer(player).getCards(), CharacterCard.class)) {
            characterCards.add(card);
            hovers.add(card.toString());
        }
        characterCardsPanel.setImages(Images.getImagesForCards(characterCards), hovers);

        List<WeaponCard> weaponCards = new ArrayList<>();
        hovers.clear();
        for (WeaponCard card : filter(board.getPlayer(player).getCards(), WeaponCard.class)) {
            weaponCards.add(card);
            hovers.add(card.toString());
        }
        weaponCardsPanel.setImages(Images.getImagesForCards(weaponCards), hovers);

        List<RoomCard> roomCards = new ArrayList<>();
        hovers.clear();
        for (RoomCard card : filter(board.getPlayer(player).getCards(), RoomCard.class)) {
            roomCards.add(card);
            hovers.add(card.toString());
        }
    ...
    public void viewKeeperCards() {
        ...
        if (user.getKeeperCards().size() == 0)
            ...
            for (KeeperCard card : user.getKeeperCards()) {
                ...
                int quantity =
                        Iterables.size(Iterables.filter(user.getKeeperCards(), card.getClass()));

                sb.append(String.format("%s x%s%n\t%s%n", card.getName(), quantity,
                        card.getDescription()));
            }
        }
    ...
    public void viewNotebook() {
        StringBuilder sb =
                new StringBuilder(board.getPlayer(player).toString()).append("'s Notebook:");

        Notebook notebook = board.getPlayer(player).getNotebook();

        sb.append("\n\n").append("Unseen Character Cards:");
        for (CharacterCard card : filter(notebook.availableCards(), CharacterCard.class))
            sb.append("\n").append(card);

        sb.append("\n\n").append("Unseen Weapon Cards:");
        for (WeaponCard card : filter(notebook.availableCards(), WeaponCard.class))
            sb.append("\n").append(card);
        ...
        JOptionPane.showMessageDialog(getParent(), sb, "Notebook", JOptionPane.INFORMATION_MESSAGE);
    }
}
