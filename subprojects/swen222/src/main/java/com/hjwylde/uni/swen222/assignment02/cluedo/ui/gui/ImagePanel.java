 ...
 * that is used as a tool tip.
 * TODO: The panel is hardcoded with values to work with the cards and current GUI - this should be
 * changed for a fluid layout.
 * 
...
public class ImagePanel extends JPanel {
    ...
    public void setImages(Image[] images, String[] hovers) {
        checkArgument(images.length == hovers.length,
            "Can't give two arrays of different sizes!");
        
        removeAll();
        for (int i = 0; i < images.length; i++) {
            JLabel label = new JLabel(new ImageIcon(
                images[i].getScaledInstance(47, 70, Image.SCALE_FAST)));
