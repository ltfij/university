...
public final class KeeperCards {
     ...
     */
    public <T extends KeeperCard> ImmutableList<Integer> findPlayersWithKeeperCard(
        Class<T> clazz, Collection<Integer> ignore) {
    ...
    public boolean queryPlayerMoveAnywhere(int player) {
        if (!queryPlayerPlayKeeperCard(board.getPlayer(player),
            MoveAnywhere.class))
            return false;
     ...
     */
    public <T extends KeeperCard> boolean queryPlayerPlayKeeperCard(
        Player player, Class<T> clazz) {
        Optional<T> card = getPlayerKeeperCard(player, clazz);
        
        return card.isPresent()
            && (ui.queryPlayerPlayKeeperCard(player, card.get()) == Option.YES);
    }
    ...
    public boolean queryPlayerPlayMovePlayerToStart(int player) {
        ...
        Character character = ui.queryPlayerMovePlayerToStart(
            board.getPlayer(player), options);
        
        board.movePlayer(board.findPlayer(character).get(), board
            .getCharacterInitialPosition(character).get());
    ...
    public boolean queryPlayerPlayViewPlayerCard(int player) {
        ...
        Card card = ui.queryPlayerShowCard(board.getPlayer(rightPlayer),
            board.getPlayer(player), board.getPlayer(rightPlayer).getCards());
     ...
     */
    public static <T extends KeeperCard> Optional<T> getPlayerKeeperCard(
        Player player, Class<T> clazz) {
        Iterator<T> it = Iterables.filter(player.getKeeperCards(), clazz)
            .iterator();
     ...
     */
    public static <T extends KeeperCard> boolean playerContainsKeeperCard(
        Player player, Class<T> clazz) {
