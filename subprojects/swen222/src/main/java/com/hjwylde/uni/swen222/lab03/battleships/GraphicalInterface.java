...
public class GraphicalInterface extends JFrame implements WindowListener, MouseListener {
    ...
    /**
     * The following fields cache various icons so we don't need to load them everytime.
     */
                    ...
                    if (e.getComponent() == rightBattleGrid[x][y]) {
                        ...
                        if (isOver) {
                            ...
                            dialog.setTitle("Game over! "
                                    + (game.didPlayerWin() ? "You won!" : "You lost!"));
                            dialog.show();
                        }
                    }
    ...
    /**
     * This method is called when the user clicks on the "X" button in the right-hand corner.
     * 
     * @param e
     */
    @Override
    public void windowClosing(WindowEvent e) {
        ...
        int r =
                JOptionPane.showConfirmDialog(this, new JLabel("Exit BattleShips?"),
    ...
    private void drawBoard() {
        for (int x = 0; x < game.getWidth(); x++)
            for (int y = 0; y < game.getHeight(); y++) {
                leftBattleGrid[x][y].setIcon(getSquareIcon(game.getLeftSquare(x, y), true));
                rightBattleGrid[x][y].setIcon(getSquareIcon(game.getRightSquare(x, y), false));
            }
    }
    ...
    private void makeOutermostPanel(int width, int height) {
        ...
        JPanel rightBattlePanel = new JPanel(new GridLayout(width, height, 1, 1));
