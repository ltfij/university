 ...
 * @since 31/08/2013
 */
public final class BoardCanvas extends JPanel implements KeyListener,
    MouseListener {
    ...
    public synchronized void keyPressed(KeyEvent e) {
        if ((e.getModifiers() != 0) || (player < 0)
            || ((movesLeft <= 0) && !moveAnywhere) || (turnStart < 0)
    ...
    public synchronized void mouseClicked(MouseEvent e) {
        if ((player < 0) || ((movesLeft <= 0) && !moveAnywhere)
    ...
    public void paint(Graphics g) {
        ...
        if (moveAnywhere)
            ...
            Coordinate pos = board.getPlayerPosition(player);
            paintValidMoves(g2d,
                new Coordinate(pos.getX() - movesLeft, pos.getY() - movesLeft),
                new Coordinate(pos.getX() + movesLeft + 1, pos.getY()
                    + movesLeft + 1), movesLeft);
        }
    }
    ...
    public Coordinate queryPlayerMoveAnywhere(Player player) {
        checkState(player.equals(board.getPlayer(this.player)));
        checkState(moveAnywhere,
            "Logic should not call move anywhere unless we actually can!");
     ...
     * @param g the graphics.
     */
    private void paintValidMoves(Graphics2D g, Coordinate start,
        Coordinate end, int movesLeft) {
        Coordinate pos = board.getPlayerPosition(player);
        ...
        for (int x = start.getX(); x < end.getX(); x++)
            for (int y = start.getY(); y < end.getY(); y++) {
                ...
                g.drawRect(tile.getX(), tile.getY(), getTileWidth(),
                    getTileHeight());
            }
    }
