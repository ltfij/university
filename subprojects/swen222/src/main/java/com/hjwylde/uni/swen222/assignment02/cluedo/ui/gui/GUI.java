...
public final class GUI implements UI, Runnable {
    ...
    public void informPlayerClockCardLoss(Player player) {
        inform("Player Lost", player + " has just lost the game by picking up the 8th clock card!");
    }
    ...
    public void informPlayerPickUpClockCard(Player player, int remaining) {
        inform("Intrigue Card", player + " has just picked up a clock card! There are now "
                + remaining + " left.");
    }
    ...
    public void informPlayerPickUpKeeperCard(Player player, KeeperCard card) {
        inform("Intrigue Card", player + " has just picked up the " + card + " keeper card!");
    }
     ...
     */
    @Override
    public void informPlayerWin(Player player, Triple<Character, Weapon, Room> accusation) {
        inform("Player Win", "Congratulations " + player + "! You have just won the game!" + "\n"
                + "It was " + accusation.getFirst() + " with the " + accusation.getSecond()
                + " in the " + accusation.getThird() + ".");
    }
    ...
    public void informPlayerWrongAccusationLoss(Player player) {
        inform("Player Lost", player + " has just lost by making an incorrect accusation!");
    }
    ...
    public int queryNumberPlayers(List<Integer> options) {
        return query("Number of Players", "Please select the number of players for this game:",
                options);
    }
     ...
     */
    @Override
    public Card queryPlayerChooseRefuteRumourCard(Player player, Set<Card> options) {
        return query("Rumour", player + ", please select a card to refute the rumour with:",
                new ArrayList<>(options), Images.getImagesForCards(new ArrayList<>(options)));
    }
     ...
     */
    @Override
    public Triple<Character, Weapon, Room> queryPlayerMakeAccusation(Player player) {
        Character character =
                query("Accusation", player + ", please select a character to accuse:",
                        Character.values(), Images.getImagesForCharacters(Character.values()));
        Weapon weapon =
                query("Accusation", player + ", please select a weapon for the accusation:",
                        Weapon.values(), Images.getImagesForWeapons(Weapon.values()));
        Room room =
                query("Accusation", player + ", please select a room for the accusation:",
                        Room.murderRoomValues(), Images.getImagesForRooms(Room.murderRoomValues()));
     ...
     */
    @Override
    public Character queryPlayerMovePlayerToStart(Player player, List<Character> options) {
        return query("Move Player to Start", player + ", who would you like to move to the start?",
                options, Images.getImagesForCharacters(options));
    }
    ...
    public String queryPlayerName(int player) {
        ...
        do {
            name =
                    JOptionPane.showInputDialog(frame, "Player #" + (player + 1)
            ...
            JOptionPane.showMessageDialog(frame, "Sorry, but you must enter a valid name.",
    ...
    public Option queryPlayerPlayKeeperCard(Player player, KeeperCard card) {
        return query("Play Keeper Card", player + ", would you like to play the " + card
                + " keeper card? (" + card.getShortDescription() + ")", Option.values());
    }
     ...
     */
    @Override
    public Card queryPlayerShowCard(Player playerFrom, Player playerTo, Set<Card> options) {
        return query("Show Card", playerFrom + ", please select a card to show " + playerTo + ":",
                new ArrayList<>(options), Images.getImagesForCards(new ArrayList<>(options)));
    }
    ...
    public Pair<Character, Weapon> queryPlayerStartRumour(Player player) {
        Character character =
                query("Rumour", player + ", please select a character for the rumour:",
                        Character.values(), Images.getImagesForCharacters(Character.values()));
        Weapon weapon =
                query("Rumour", player + ", please select a weapon for the rumour:",
                        Weapon.values(), Images.getImagesForWeapons(Weapon.values()));
     ...
     */
    @Override
    public PlayerTurnChoice queryPlayerTurnChoice(Player player, Set<PlayerTurnChoice> options) {
    ...
    private <T> T query(String title, String query, List<T> options) {
        checkArgument(options.size() > 0, "Must be at least one option for the user!");
        ...
        do {
            ...
            JOptionPane.showMessageDialog(frame, "Sorry, but you must select a valid option.",
                    "Invalid Option", JOptionPane.ERROR_MESSAGE);
        } while (true);
    }

    private <T> T query(String title, String query, List<T> options, List<Image> images) {
        checkArgument(options.size() > 0, "Must be at least one option for the user!");
    ...
    private <T> T query(String title, String query, T[] options, Image[] images) {
        return query(title, query, Arrays.asList(options), Arrays.asList(images));
    }
}
