package com.hjwylde.uni.comp422.project02.question04_2;

import static com.google.common.base.Preconditions.checkNotNull;

import com.hjwylde.uni.comp422.project02.question04_1.Datum;
...
public final class Main {
    ...
    private static final String ROOT_PATH = "/project02/question04_2/";
    private static final String PIMA_TRAIN_PATH = ROOT_PATH + "pima-train.data";
    private static final String PIMA_TEST_PATH = ROOT_PATH + "pima-test.data";

    private static final List<Datum> PIMA_TRAIN_DATA;
    private static final List<Datum> PIMA_TEST_DATA;

    static {
        try {
            InputStream in = checkNotNull(Main.class.getResourceAsStream(PIMA_TRAIN_PATH),
                    "in cannot be null");
            PIMA_TRAIN_DATA = loadData(in);
            in = checkNotNull(Main.class.getResourceAsStream(PIMA_TEST_PATH), "in cannot be null");
            PIMA_TEST_DATA = loadData(in);
        } catch (IOException e) {
    ...
    private double run() throws InvalidConfigurationException {
        ...
        System.out.println(fitness(pop.determineFittestProgram(), PIMA_TRAIN_DATA));
        System.out.println(fitness(pop.determineFittestProgram(), PIMA_TEST_DATA));

        double fitness = fitness(pop.determineFittestProgram(), PIMA_TEST_DATA);
        double size = PIMA_TEST_DATA.size();
        return fitness / size;
    ...
    private class FormulaFitnessFunction extends GPFitnessFunction {
        ...
        public double computeRawFitness(final IGPProgram ind) {
            return fitness(ind, PIMA_TRAIN_DATA);
        }
