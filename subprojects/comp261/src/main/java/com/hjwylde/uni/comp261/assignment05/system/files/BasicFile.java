...
public class BasicFile {
    ...
    public boolean open(String fileName, boolean createIfDoesNotExist) {
         ...
         * FILE<> header.
         */
        if (fileName.equals("") || (fileName.length() > (BasicFile.BLOCK_SIZE - 20))
            ...
            for (int j = 0; j <= fileName.length(); j++)
                ...
                if ((buffer[j + 5] == '>') && (j == fileName.length())) { // If we have reached the
                    ...
                    return true;
                                                                                               ...
                                                                                               // don't
                                                                                               // match...
                    while (((j + 5) < BasicFile.BLOCK_SIZE) && (buffer[j + 5] != '>'))
        ...
        if (createIfDoesNotExist) { // Create new file?
            char[] buffer =
                    Arrays.copyOf(("FILE<" + fileName + ">0").toCharArray(), BasicFile.BLOCK_SIZE); // Create
                                                                                                    // a
                                                                                                    // new
                                                                                                    // character
                                                                                                    // array
    ...
    public boolean write(char[] data) {
        ...
        // the eof.
        int insert =
                ((filePointer + data.length) / BasicFile.BLOCK_SIZE) - (eof / BasicFile.BLOCK_SIZE);
    ...
    private Disk.Block getBlock(int filePointer) {
        return disk.getBlock(1 + fileHandle + (filePointer / BasicFile.BLOCK_SIZE)); // Convert into
                                                                                     // block index
        // then return.
    }
    ...
    private void updateNumBlocks() {
        ...
        System.arraycopy(numBlocks, 0, buffer, fileSizePointer, numBlocks.length); // Write the
                                                                                   ...
                                                                                   // the
                                                                                   // header.
        Arrays.fill(buffer, fileSizePointer + numBlocks.length, BasicFile.BLOCK_SIZE,
