...
public class SequentialFile {
    ...
    public Record find(int id) {
        ...
        return SequentialFile.readRecord(getBlock(index).get(), relativeFilePointer()); // Return
                                                                                        // the
                                                                                        // record.
    }
    ...
    public boolean open(String fileName, boolean createIfDoesNotExist) {
         ...
         * FILE<> header.
         */
        if (fileName.equals("") || (fileName.length() > (SequentialFile.BLOCK_SIZE - 20))
            ...
            for (int j = 0; j <= fileName.length(); j++)
                ...
                if ((buffer[j + 5] == '>') && (j == fileName.length())) { // If we have reached the
                    ...
                    return true;
                                                                                               ...
                                                                                               // don't
                                                                                               // match...
                    while (((j + 5) < SequentialFile.BLOCK_SIZE) && (buffer[j + 5] != '>'))
        ...
        if (createIfDoesNotExist) { // Create new file?
            char[] buffer =
                    Arrays.copyOf(("FILE<" + fileName + ">0").toCharArray(),
                            SequentialFile.BLOCK_SIZE); // Create a new character array with correct
                                                        // header!
    ...
    public boolean write(Record r) {
        ...
        SequentialFile.writeRecord(r, getBlock(index).get(), relativeFilePointer()); // Write the
                                                                                     // record.
    ...
    private void deleteRecord(int index) {
        ...
        for (int i = eof; i >= index; i--) { // For every record from eof to index...
            ...
            char[] holder =
                    Arrays.copyOfRange(buffer, relativeFilePointer(), relativeFilePointer()
                            + SequentialFile.RECORD_SIZE); // Temp.
            System.arraycopy(record, 0, buffer, relativeFilePointer(), SequentialFile.RECORD_SIZE); // Move
                                                                                                    // the
                                                                                                    // previous
            // record down.
        ...
        buffer = getBlock((SequentialFile.BLOCKING_FACTOR * numBlocks) - 1).get(); // Get the last
                                                                                   // block.
    ...
    private Disk.Block getBlock(int filePointer) {
        return disk.getBlock(1 + fileHandle + (filePointer / SequentialFile.BLOCKING_FACTOR)); // Convert
                                                                                               // into
                                                                                               // block
        // index then return.
    }
    ...
    private int indexOf(int id) {
        ...
        int index = indexOf(Main.toCharArray(id, Record.ID_LENGTH), 0, eof() + 1); // Binary search!
    ...
    private void insertRecord(int index) {
        ...
        for (int i = index; i < (SequentialFile.BLOCKING_FACTOR * numBlocks); i++) { // For each
            ...
            char[] holder =
                    Arrays.copyOfRange(buffer, relativeFilePointer(), relativeFilePointer()
                            + SequentialFile.RECORD_SIZE);
            System.arraycopy(record, 0, buffer, relativeFilePointer(), SequentialFile.RECORD_SIZE);
            record = holder;
        }
    }
    ...
    private void setFilePointer(int index) {
        filePointer =
                (SequentialFile.RECORD_SIZE * (index % SequentialFile.BLOCKING_FACTOR))
    ...
    private void updateNumBlocks() {
        ...
        System.arraycopy(numBlocks, 0, buffer, fileSizePointer, numBlocks.length); // Write the
                                                                                   ...
                                                                                   // the
                                                                                   // header.
        Arrays.fill(buffer, fileSizePointer + numBlocks.length, SequentialFile.BLOCK_SIZE,
                (char) Character.UNASSIGNED); // Pad any space after the
                                              // number
    ...
    private static void writeRecord(Record r, char[] buffer, int to) {
        System.arraycopy(r.toCharArray(), 0, buffer, to, SequentialFile.RECORD_SIZE);
    }
