...
public class BooleanConditionNode implements Node {
    ...
    /**
     * Evaluates this <code>BooleanConditionNode</code> and returns its boolean value.
     * 
    ...
    public boolean evaluate(Map<VariableNode, Integer> variables, int robotID, World world) {
        switch (type) {
            ...
            default: // VALUE
                switch (opType) {
                    case EQUALS:
                        return firstVal.evaluate(variables, robotID, world) == secVal.evaluate(
                                variables, robotID, world);
                    case NOT_EQUALS:
                        return firstVal.evaluate(variables, robotID, world) != secVal.evaluate(
                                variables, robotID, world);
                    case GREATER_THAN:
