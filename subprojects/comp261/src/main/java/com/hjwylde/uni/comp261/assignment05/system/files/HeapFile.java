...
public class HeapFile {
    ...
    private static final int BLOCKING_FACTOR = HeapFile.BLOCK_SIZE / HeapFile.RECORD_SIZE;
    ...
    public Record find(int id) {
        ...
        return HeapFile.readRecord(getBlock(index).get(), relativeFilePointer()); // Return the
                                                                                  // record.
    }
    ...
    public boolean open(String fileName, boolean createIfDoesNotExist) {
         ...
         * FILE<> header.
         */
        if (fileName.equals("") || (fileName.length() > (HeapFile.BLOCK_SIZE - 20))
                || fileName.contains("<") || fileName.contains(">") || fileName.contains("~"))
            return false;

        blockFor: for (int i = 0; i < disk.getNumberOfBlocks(); i++) { // For every block on the
                                                                       // disk...
            ...
            for (int j = 0; j <= fileName.length(); j++)
                ...
                if ((buffer[j + 5] == '>') && (j == fileName.length())) { // If we have reached the
                    ...
                    return true;
                } else if ((j == fileName.length()) || (buffer[j + 5] != fileName.charAt(j))) {// ...
                                                                                               // else
                                                                                               // if
                                                                                               // characters
                                                                                               // don't
                                                                                               // match...
                    while (((j + 5) < HeapFile.BLOCK_SIZE) && (buffer[j + 5] != '>'))
        ...
        if (createIfDoesNotExist) { // Create new file?
            char[] buffer =
                    Arrays.copyOf(("FILE<" + fileName + ">0").toCharArray(), HeapFile.BLOCK_SIZE); // Create
                                                                                                   // a
                                                                                                   // new
                                                                                                   // character
                                                                                                   // array
    ...
    public void sort() {
        ...
        /*
         * NOT FINISHED! I did not manage to do the merge phase of this sorting algorithm, wasn't
         * sure how / ran out of time. The code I started to try is still there but it doesn't
         * really work well. The sort runs part will just sort each of the runs where each run has n
         * - 1 blocks.
         */
    ...
    private Disk.Block getBlock(int filePointer) {
        return disk.getBlock(1 + fileHandle + (filePointer / HeapFile.BLOCKING_FACTOR)); // Convert
                                                                                         // into
                                                                                         // block
        // index then return.
    }
    ...
    private int indexOf(int id) {
        ...
        for (int i = 0; i < (numBlocks * HeapFile.BLOCKING_FACTOR); i++) { // Go through every block
             ...
             */
            if (Arrays.equals(
                    Arrays.copyOfRange(buffer, relativeFilePointer(), relativeFilePointer()
                            + Record.ID_LENGTH), Main.toCharArray(id, Record.ID_LENGTH)))
    ...
    private void mergeRuns(int n) {
        ...
        for (int i = 1 + (numBlocks / (n - 1)); i > 1; i /= (n - 1)) {
            /*
             * I tried: To iterate through each subfile (note: at the moment writing the subfiles to
             * the end of the disk is disabled in the sortRuns() method) and create a sort of stack
             * like buffer using an iterator that has a peek() method for each run.
             */
            List<PeekIterator> runIts = new ArrayList<PeekIterator>();
            for (int run = 0; run < i; run++)
                // Create all the PeekIterators.
                runIts.add(new PeekIterator(blockEof + (run * (n - 1)), 1 + (numBlocks / i)));

            /*
             * Go through all of the iterators in total until none are left, adding the lowest
             * poll() of them to a subfile written at the bottom of the disk.
             */
            while (!runIts.isEmpty()) {
                ...
                HeapFile.writeRecord(runIts.get(low).next(),
                        disk.getBlock(disk.getNumberOfBlocks() - 1).get(), HeapFile.RECORD_SIZE
    ...
    private void sortRuns(int n) {
        PriorityQueue<Record> queue =
                new PriorityQueue<>(HeapFile.BLOCKING_FACTOR * (n - 1), new RecordSorter()); // Priority
                                                                                             // queue
                                                                                             // for
                                                                                             // sorting.
        /*
         * Note: I didn't use any other buffer really apart from the priority queue as a combination
         * of them all... just felt this would be easier for sorting them.
         */
        ...
        for (int run = 0; run < numBlocks; run += (n - 1)) { // For each run...
            ...
            bufferLoop: for (int buffer = 0; buffer < (n - 1); buffer++) { // For each buffer in the
                                                                           // run...
                ...
                for (int index = block * HeapFile.BLOCKING_FACTOR; index < ((1 + block) * HeapFile.BLOCKING_FACTOR); index++) { // For
                    ...
                    queue.offer(HeapFile.readRecord(blockBuffer, relativeFilePointer())); // Offer
                                                                                          // the
                                                                                          // record
                                                                                          // to the
                    // runs queue.
            ...
            /*
             * Records are now sorted for the run, write them out. I have commented out the writing
             * the runs as subfiles at the end of the disk as the merge phase doesn't work. Instead
             * for now they just write the sorted runs back to the file.
             */

            bufferLoop: for (int buffer = 0; buffer < (n - 1); buffer++) {
                ...
                for (int index = block * HeapFile.BLOCKING_FACTOR; index < ((1 + block) * HeapFile.BLOCKING_FACTOR); index++) {
                    ...
                    HeapFile.writeRecord(queue.poll(), getBlock(index).get(), relativeFilePointer()); // Write
                                                                                                      // back
                                                                                                      // to
                                                                                                      // the
                                                                                                      // correct
                ...
                /*
                 * Disk.Block diskBlock = new Disk.Block(); diskBlock.set(blockBuffer);
                 * disk.addBlock(diskBlock); blockBuffer = null;
                 */
            }

            /*
             * if (blockBuffer != null) { Disk.Block diskBlock = new Disk.Block();
             * diskBlock.set(blockBuffer); disk.addBlock(diskBlock); }
             */
        }
    }
    ...
    private void updateNumBlocks() {
        ...
        System.arraycopy(numBlocks, 0, buffer, fileSizePointer, numBlocks.length); // Write the
                                                                                   // number
                                                                                   // of blocks to
                                                                                   // the
                                                                                   // header.
        Arrays.fill(buffer, fileSizePointer + numBlocks.length, HeapFile.BLOCK_SIZE,
                (char) Character.UNASSIGNED); // Pad any space after the number of
    ...
    private static Record readRecord(char[] buffer, int from) {
        ...
        r.name =
                Arrays.copyOfRange(buffer, from + Record.ID_LENGTH, from + Record.ID_LENGTH
                        + Record.NAME_LENGTH);
        r.grade =
                Arrays.copyOfRange(buffer, from + Record.ID_LENGTH + Record.NAME_LENGTH, from
                        + Record.ID_LENGTH + Record.NAME_LENGTH + Record.GRADE_LENGTH);
    ...
    public static class Record {
        ...
        public char[] toCharArray() {
            ...
            System.arraycopy(Main.toCharArray(id, Record.ID_LENGTH), 0, record, 0, Record.ID_LENGTH);
            System.arraycopy(name, 0, record, Record.ID_LENGTH, Record.NAME_LENGTH);
            System.arraycopy(grade, 0, record, Record.ID_LENGTH + Record.NAME_LENGTH,
                    Record.GRADE_LENGTH);
        ...
        public String toString() {
            return "{(" + id + "), (" + Arrays.toString(name) + "), (" + Arrays.toString(grade)
                    + ")}";
        }
    ...
    private class PeekIterator implements Iterator<HeapFile.Record> {
        ...
        public boolean hasNext() {
            ...
            return (buffer[HeapFile.RECORD_SIZE * (index % HeapFile.BLOCKING_FACTOR)] != Character.UNASSIGNED);
        }
        ...
        public HeapFile.Record next() {
            ...
            HeapFile.Record next =
                    HeapFile.readRecord(buffer, HeapFile.RECORD_SIZE
            ...
            if (((index % HeapFile.BLOCKING_FACTOR) == 0)
                ...
                buffer = disk.getBlock(blockIndex + (index / HeapFile.BLOCKING_FACTOR)).get();
