...
import com.hjwylde.uni.comp261.assignment04.graphics.*;
import com.hjwylde.uni.comp261.assignment04.graphics.ScanConverter.Scan;
...
public class Polytope2d implements Polytope, Iterable<Vector3d> {
    ...
    /**
     * Creates a new <code>Polytope2d</code> with vertices from the given <code>Collection</code>
     * and sets its color to be the given <code>Color</code>.
     * 
    ...
    public boolean clip(double clipZ) {
        ...
        for (int i = 0; i < vertices.size(); i++) { // For each vertex...
            ...
            if ((v1.z < clipZ) && (v2.z > clipZ)) { // If this edge intersects with the clip
                ...
                vertices.add(next, new Vector3d(v1.x + (scale * (v2.x - v1.x)), v1.y
                        + (scale * (v2.y - v1.y)), clipZ)); // ... create a new vertex at the
                                                            // intersection
                                                            // point.
     ...
     * normal and the direction vector to <code>u</code>. If the dot product is greater than 0, then
     * the angle between the normal and direction vector <code>u - vertices.get(0)</code> is between
     * 0 and 90 degrees (ie. it is facing).
     * 
    ...
    public double getShade(Vector3d v, List<PointLight3d> lights) {
        if (!World3d.renderLightsMode || !vertexNormals.containsKey(v) || (lights == null)
                || (lights.size() == 0))
        ...
        for (PointLight3d light : lights)
            shade *=
                    World3d.ambientLight + (light.getIntensity() * vertexNormals.get(v).dot(light));
    ...
    public void project(ViewPlane view) {
        /*
         * Perspective transform all vertices x and y co-ordinates onto the ViewPlane. Best to use a
         * scratch Polytope for this so as to not overwrite the original vertices.
         */
        for (Vector3d v : vertices) {
        ...
        for (int y = sc.getTopBoundary(); y <= sc.getBottomBoundary(); y++) { // For each scan.
            ...
            if (World3d.wireframeMode) { // If wireframe mode...
                z.set(s.getLeft(), y, s.getLeftDepth(),
                        Polytope2d.getShadedColor(color, s.getLeftShade())); // Draw the left pixel
                                                                             // of
                                                                             // the scan.
                z.set(s.getRight(), y, s.getRightDepth(),
                        Polytope2d.getShadedColor(color, s.getRightShade())); // Draw the right
                                                                              // pixel of
                                                                              // the
                                                                              // scan.
                 ...
                 * pixel width scans then the face would be drawn with a black dot at the point of
                 * each vertex.
                 */
                z.set(s.getLeft(), y, s.getLeftDepth(),
                        Polytope2d.getShadedColor(color, s.getLeftShade())); // Draw the single
                                                                             // pixel.
            else { // ... else draw the scan line.
                double depthGradient =
                        (s.getRightDepth() - s.getLeftDepth()) / (s.getRight() - s.getLeft()); // For
                                                                                               // linearly
                                                                                               // interpolating
                                                                                               // the
                                                                                               // depth.
                double shadeGradient =
                        (s.getRightShade() - s.getLeftShade()) / (s.getRight() - s.getLeft()); // For
                                                                                               // linearly
                                                                                               // interpolating
                                                                                               // the
                                                                                               // shade.
                ...
                for (int x = s.getLeft(); x <= s.getRight(); x++) {
                    ...
                    z.set(x, y, s.getLeftDepth() + ((x - s.getLeft()) * depthGradient),
                            Polytope2d.getShadedColor(color, shade)); // Set the pixel at (x, y) on
                                                                      // the
                                                                      // zBuffer.
                }
            }
    ...
    private static Color getShadedColor(Color color, double shade) {
        return new Color((int) Main.constrain((color.getRed() * shade), 0, 255),
                (int) Main.constrain((color.getGreen() * shade), 0, 255), (int) Main.constrain(
                        (color.getBlue() * shade), 0, 255));
    }
}
