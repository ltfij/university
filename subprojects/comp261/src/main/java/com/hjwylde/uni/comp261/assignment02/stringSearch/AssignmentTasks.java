...
/*
 * Code for Assignment 2, COMP 261 Name: Henry J. Wylde Usercode: wyldehenr ID: 300224283
 */
...
public class AssignmentTasks {
    ...
    public AssignmentTasks() {
        if (!loadDocuments()) {
            JOptionPane.showMessageDialog(null, "Error loading documents.", "Error",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (!loadPhrases()) {
            JOptionPane.showMessageDialog(null, "Error loading phrases.", "Error",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    public void taskOne() {
        ...
        System.out.println("Total time for Boyer Moore: " + (System.currentTimeMillis() - curTime)
                + "ms.");
        curTime = System.currentTimeMillis();

        /*
         * KMP: This is just a repeat of the above code using KMP instead, to test / compare the
         * times of KMP and Boyer Moore.
         */

        // Knuth Morris Pratt

        /*
         * finder = new StringSearch.KnuthMorrisPrat(); phrasesCount = new HashMap<File,
         * List<Integer>>(); for (Map.Entry<File, String> e : documents.entrySet()) { String
         * document = e.getValue(); List<Integer> counts = new
         * ArrayList<Integer>(phrasesList.size()); for (int i = 0; i < phrasesList.size(); i++) {
         * String haystack = document; String phrase = phrasesList.get(i); int count = 0; int index
         * = finder.find(document, phrase); while (index != -1) { count++; if (index + 1 >
         * haystack.length()) break; haystack = haystack.substring(index + 1); index =
         * finder.find(haystack, phrase); } counts.add(count); } phrasesCount.put(e.getKey(),
         * counts); } System.out.println("Total time for Knuth Morris Pratt: " +
         * (System.currentTimeMillis() - curTime) + "ms."); curTime = System.currentTimeMillis();
         */
        ...
        for (Map.Entry<File, String> e : documents.entrySet()) {
            ...
            Map<CharSequence, List<Integer>> phraseIndices = phrasesTrie.search(document);
            for (int i = 0; i < phrasesList.size(); i++) {
                String phrase = phrasesList.get(i);

                counts.add((phraseIndices.containsKey(phrase)) ? phraseIndices.get(phrase).size()
                        : 0);
            }
        ...
        System.out
                .println("Total time for Trie: " + (System.currentTimeMillis() - curTime) + "ms.");
        ...
        try (BufferedWriter bw =
                new BufferedWriter(new FileWriter(new File(rootDirectory, "Task One Counts.tab")))) {
    ...
    public void taskTwo() {
        ...
        Queue<File> similarDocuments =
                new PriorityQueue<>(phrasesCount.size(), new Comparator<File>() {
        ...
        for (Map.Entry<File, List<Integer>> e : phrasesCount.entrySet()) {
            ...
            for (Map.Entry<File, List<Integer>> other : phrasesCount.entrySet()) {
                ...
                csMap.put(other.getKey(),
                        AssignmentTasks.cosineSimilarity(e.getValue(), other.getValue()));
    ...
    private boolean loadDocuments() {
        File dir = null;
        if (JOptionPane.showConfirmDialog(null, "Select root directory for text files.",
    ...
    private boolean loadPhrases() {
        File phrases = null;
        if (JOptionPane.showConfirmDialog(null, "Select phrases file.", "File Loading",
