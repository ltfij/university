...
public class VariableNode implements Node {
    ...
    /**
     * Constructs a <code>VariableNode</code> with the specified name. Throws an error if the name
     * doesn't match <code>NAME_REGEX</code>.
     * 
    ...
    public VariableNode(String name) throws ParseException {
        if (!name.matches(VariableNode.NAME_REGEX))
            throw new ParseException("Illegal variable name \"" + name + "\". Does not match \""
                    + VariableNode.NAME_REGEX + "\".");
