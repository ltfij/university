...
public class Tree implements Shape {
    ...
    /**
     * Constructor with explicit values Arguments are the x and y of the top left corner, the width
     * and height, and the color.
     * 
    ...
    /**
     * [Extension] Constructor which reads values from a file (scanner) The argument is a Scanner
     * that contains the specification of the Rectangle. The next 7 integers specify the position of
     * the top left corner, and the width and height, and three ints specifying the color.
     * 
     * @param data the scanner.
     */
    public Tree(Scanner data) {
        this(data.nextInt(), data.nextInt(), data.nextInt(), data.nextInt(), new Color(
                data.nextInt(), data.nextInt(), data.nextInt()), data.nextLong());
    }
    ...
    public String toString() {
        return ("Tree " + p1.x + " " + p1.y + " " + p2.x + " " + p2.y + " " + color.getRed() + " "
                + color.getGreen() + " " + color.getBlue() + " " + seed);
    }

    /**
     * A recursive function for adding branches of the Tree to an ArrayList. Takes the starting
     * position, length and angle of the branch to be added as parameters. Then calls itself 3
     * times: with a shortened length and various angles.
     * 
     * @param x0 the x co-ordinate of the branch.
     * @param y0 the y co-ordinate of the branch.
    ...
    private void addBranch(double x0, double y0, double length, double angle) {
        ...
        // lengths and angles.
        addBranch(x1, y1, length * 0.75, angle + (Math.PI / 6) + ((r.nextDouble() * Math.PI) / 9));
        addBranch(x1, y1, length * 0.4, angle + ((r.nextDouble() * Math.PI) / 18));
        addBranch(x1, y1, length * 0.66, (angle - ((Math.PI / 18) * 5))
                + ((r.nextDouble() * Math.PI) / 9));
    }

    /**
     * Creates a tree using a recursive function based on the length of the base trunk of the tree
     * (p1 and p2). Adds all the branches and leaves (5 pixel dots at end of last branches) to an
     * ArrayList.
     */
