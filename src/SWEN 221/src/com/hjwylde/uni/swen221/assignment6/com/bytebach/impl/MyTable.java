package com.hjwylde.uni.swen221.assignment6.com.bytebach.impl;

import java.util.*;

import com.hjwylde.uni.swen221.assignment6.com.bytebach.Main;
...
public class MyTable implements Table {
     ...
     * null,
     * or if there exists a reference field that references a non-existant table.
     * 
        ...
        for (Table t : db.tables()) {
            ...
            for (int i = 0; i < t.fields().size(); i++) {
                ...
                for (List<Value> r : t.rows())
                    if (Arrays.asList(((ReferenceValue) r.get(i)).keys())
                        .equals(rowKeyFieldValues))
                        toDelete.add(r);
            }
            
            // Delete them all.
            for (List<Value> r : toDelete)
                t.delete(MyTable.getRowFieldValues(
                    MyTable.getKeyFieldIndices(t.fields()), r).toArray(
                    new Value[1]));
        }
    }
        ...
        for (Table t : db.tables()) {
            ...
            for (int i = 0; i < t.fields().size(); i++) {
                ...
                for (List<Value> row : t.rows())
                    if (Arrays.asList(((ReferenceValue) row.get(i)).keys())
                        .equals(oldRowKeyFieldValues))
                        row.set(i, new ReferenceValue(name,
                            newRowKeyFieldValues.toArray()));
     ...
     */
    public static List<Value> getRowFieldValues(List<Integer> fieldIndices,
        List<Value> row) {
    ...
    private class RowList extends AbstractList<List<Value>> {
        ...
        public List<Value> set(int index, List<Value> row) {
            ...
            // one we are setting.
            if (containsRow(getRowKeyFieldValues(row)))
                if (!getRowKeyFieldValues(row).equals(
                    getRowKeyFieldValues(data.get(index))))
                    throw new InvalidOperation("Duplicate row.");
