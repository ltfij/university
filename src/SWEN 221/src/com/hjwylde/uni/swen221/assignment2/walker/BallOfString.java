...
public class BallOfString {
     ...
     * null
     * if this ball of string has no points.
     * 
     ...
     * from
     * the <code>from</code> point. Essentially checks whether the ball has passed the point in the
     * same direction in the past or not.
     * 
     ...
     * agrees with the
     * representation of the ball of string in that it only alters one co-ordinate by 1. Does not
     * allow null values.
     * 
    ...
    public int numberOfPasses(Point p) {
         ...
         * 
         * int count = 0;
         * while ((index = ball.subList(0, index).lastIndexOf(p)) >= 0) {
         * count++;
         * }
         */
     ...
     * has
     * come from the <code>from</code> point. Essentially this means it counts the number of times
     * it
     * has passed the point with the same direction as <code>getDirection(from, to)</code>.
     * 
     * @param from the from point.
     * @param to the to point.
    ...
    public int numberOfPasses(Point from, Point to) {
         ...
         * 
         * int count = 0;
         * while ((index = ball.subList(0, index).lastIndexOf(to)) > 0)
     ...
     * that
     * the two points only differ by either the <i>x</i> or <i>y</i> co-ordinate and that they only
     * differ by 1.
     * 
     * @param from the from point.
     * @param to the to point.
