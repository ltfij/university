package com.hjwylde.uni.swen221.assignment4.shapes;
...
public class Color {
    ...
    public Color(int red, int green, int blue) {
        if ((red < 0) || (red > 255))
            throw new IllegalArgumentException(
                "red component must take value between 0 and 255");
        if ((green < 0) || (green > 255))
            throw new IllegalArgumentException(
                "green component must take value between 0 and 255");
        if ((blue < 0) || (blue > 255))
            throw new IllegalArgumentException(
                "blue component must take value between 0 and 255");
        this.red = red;
        this.green = green;
        this.blue = blue;
    }
    ...
    public Color(String hex) {
        if ((hex.length() != 7) || (hex.charAt(0) != '#'))
            throw new IllegalArgumentException(
                "hex string requires # followed by 6 hex digits");
    ...
    public String toString() {
        return "#" + Color.hexDigit(red) + Color.hexDigit(green)
            + Color.hexDigit(blue);
    }
    
    public static Color combine(Color c1, Color c2) {
        return new Color(Color.combine(c1.red, c2.red), Color.combine(c1.green,
            c2.green), Color.combine(c1.blue, c2.blue));
    }
