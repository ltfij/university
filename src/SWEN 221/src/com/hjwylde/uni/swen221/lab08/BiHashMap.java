...
public class BiHashMap<K, V> implements Map<K, V> {
     ...
     * indicate
     * that the map contains no mapping for the key; it is also possible that the map explicitly
     * maps
     * the key to null. The containsKey method may be used to distinguish these two cases.
     * 
     * @param key the key.
     ...
     * implies
     * that t1.hashCode()==t2.hashCode() for any two maps t1 and t2, as required by the general
     ...
     * iteration
     * over the set is in progress (except through the iterator's own remove operation), the results
     ...
     * effect
     * of this call is equivalent to that of calling put(k, v) on this map once for each mapping
     * from
     * key k to value v in the specified map. The behavior of this operation is unspecified if the
     ...
     * Iterator.remove,
     * Collection.remove, removeAll, retainAll and clear operations. It does not support the add or
     * addAll operations.
     * 
