...
public class BasicMatcherTest extends BasicMatcher {
     ...
     */
    private static final String TEST_CHARS = "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890$*?+.";
     ...
     * using
     * a randomly generated string and a randomly generated regular expression.
     */
    @Test
    public void testMatch() {
        for (int i = 0; i < BasicMatcherTest.NUM_TESTS; i++)
            test(
                BasicMatcherTest.generateString((int) (Math.random() * 15)),
                BasicMatcherTest.generateRegex((int) (Math.random() * 8),
                    Math.random() < 0.30, Math.random() < 0.30));
    }
     ...
     * an
     * error too, otherwise asserts that they arrive at the same match result.
     * 
    ...
    private void test(final String text, final String regex) {
        ...
        try {
            ...
            Assert.assertTrue("{result: " + result + ", text: " + text
                + ", regex: " + regex + "}", error); // An error occured on matchWithLib, assert
                                                     // that one occured with
                                                     // match.
        }
        
        Assert.assertTrue("{result: " + result + ", libResult: " + libResult
     ...
     * regular
     * expression will begin with a carrot ('^') or end with a dollar sign ('$').
     * 
     ...
     */
    private static String generateRegex(final int length,
        final boolean beginCarrot, final boolean endDollar) {
