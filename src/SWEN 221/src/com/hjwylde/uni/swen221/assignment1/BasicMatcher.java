package com.hjwylde.uni.swen221.assignment1;
...
public class BasicMatcher implements Matcher {
    ...
    public boolean match(String text, String regex) {
        ...
        /*
         * Check if the regex should be matched straight away with the beginning of the text.
         * 
         * This is a rather poor method of checking whether the start character has a quantifier...
         * I
         * haven't improved on it as I see the problem being a fundamental problem with the grammar
         * for
         * this type of parser. I thought about how it could be done better, but it all seemed like
         * it
         * would require quite a lot of specific conditionals for special cases which isn't what it
         * should require.
         * 
         * Note: this doesn't fix the problem of having more than one begin character in the string.
         * I
         * didn't understand how the Java regex's work when given multiple carrots so I left that
         * out.
         */
        ...
        if ((regexLength > 0) && (regex.charAt(0) == '^')) { // If we begin with a start
            ...
            if (regexLength > 1)
                if ((regex.charAt(1) == '?') || (regex.charAt(1) == '*')
                    ...
                    if (regexLength > 2)
                        if ((regex.charAt(2) == '?')
                            || (regex.charAt(2) == '+')) // Second quantifier check -
                                                         // whether we are possessive
                                                         // or reluctant.
            ...
            return (consumeCarrot ? matchHere(text, restOfRegex) : match(text,
                restOfRegex)); // If we don't need to consume the carrot, don't bother and just call
                               // match
                               // again without it.
        }
     ...
     * string.
     * This is in contrast to the {@link #match(String, String)} method which attempts to match the
     * regular expression anywhere in the text string.
     * Assumes that the regular expression has been processed by the {@link #match(String, String)}
     * method already, such that it does not begin with a '^' character.
     * 
    ...
    protected boolean matchHere(String text, String regex) {
         ...
         * 
         * Allowed quantifiers:
         * '?' - matches 0..1 of the preceding character.
         * '*' - matches 0..* of the preceding character.
         * '+' - matches 1..* of the preceding character.
         */
        if (regex.length() > 1)
     ...
     * to
     * match the first character of the regular expression one or more times against the text input,
     * and then attempts to match the rest of the regular expression against the rest of the text
     * input.
     * 
     * Checks the character following the '+' to see if it is a reluctant or possessive quantifier
     * and
     * applies the appropriate logic. A possessive quantifier will consume all of the possible
    ...
    protected boolean matchPlus(String text, String regex) {
        ...
        // string.
        return ((regex.charAt(0) == '$') && (text.length() == 0) ? matchHere(
            text, restOfRegex) : false);
    }
    
    /**
     * Assumes that the start of the regex is some character followed by a '?' character. It tries
     * to
     * match the first character of the regular expression zero or one times against the text input,
     * and then attempts to match the rest of the regular expression against the rest of the text
     * input.
     * 
     * Checks the character following the '?' to see if it is a reluctant or possessive quantifier
     ...
     * Assumes that the start of the regex is some character followed by a '*' character. It tries
     * to
     * match the first character of the regular expression zero or more times against the text
     * input,
     * and then attempts to match the rest of the regular expression against the rest of the text
     * input.
     * 
    ...
    protected boolean validateRegex(String regex) {
         ...
         * Note:
         * 
         * The regular expression is only being validated for the implementations we have been told
         * to
         * make and I have allowed greedy, reluctant and possessive quantifiers as an addition (eg.
         * "*",
         * "*?" and "*+" respectively)
         */
        ...
        for (String danglingMetaSequence : danglingMetaSequences) {
            ...
            if (index >= 0)
                throw new PatternSyntaxException("Dangling meta character '"
                    + danglingMetaSequence.substring(danglingMetaSequence
                        .length() - 1) + "'", regex,
                    (index + danglingMetaSequence.length()) - 1);
        }
