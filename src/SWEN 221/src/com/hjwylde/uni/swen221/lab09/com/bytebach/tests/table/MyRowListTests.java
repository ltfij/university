...
import com.hjwylde.uni.swen221.assignment6.com.bytebach.model.InvalidOperation;
import com.hjwylde.uni.swen221.assignment6.com.bytebach.model.ReferenceValue;
import com.hjwylde.uni.swen221.assignment6.com.bytebach.model.Value;
...
public class MyRowListTests {
    ...
    public void testInvalidAdd() {
        ...
        try {
            db.table("table").rows()
                .add(-1, Arrays.asList(TableTestSuite.TABLE_ROWS[0]));
            Assert.fail();
        } catch (InvalidOperation e) {}
        try {
            db.table("table").rows()
                .add(11, Arrays.asList(TableTestSuite.TABLE_ROWS[0]));
            Assert.fail();
        } catch (InvalidOperation e) {}
        try {
            db.table("table").rows()
                .add(Arrays.asList(TableTestSuite.REF_ROWS[0]));
            Assert.fail();
        } catch (InvalidOperation e) {}
        try {
            db.table("table").rows()
                .add(Arrays.asList(TableTestSuite.TABLE_ROWS[0]));
            Assert.fail();
        } catch (InvalidOperation e) {}
    }
    ...
    public void testInvalidSet() {
        ...
        try {
            db.table("table").rows()
                .set(-1, Arrays.asList(TableTestSuite.TABLE_ROWS[0]));
            Assert.fail();
        } catch (InvalidOperation e) {}
        try {
        ...
        try {
            db.table("table").rows()
                .set(1, Arrays.asList(TableTestSuite.REF_ROWS[0]));
            Assert.fail();
        } catch (InvalidOperation e) {}
        try {
            db.table("table").rows()
        ...
        try {
            db.table("refs").rows()
                .set(0, Arrays.asList(TableTestSuite.REF_INVALID_ROWS[0]));
            Assert.fail();
        } catch (InvalidOperation e) {}
        try {
            db.table("table3")
                .rows()
                .set(0,
                    Arrays.asList(TableTestSuite.createValues(new Object[][] {
                        {
                            5, "String", "Not a boolean"
                        }
                    })[0]));
            Assert.fail();
        } catch (InvalidOperation e) {}
    ...
    public void testValidSet() {
        ...
        db.table("table").rows()
            .set(0, Arrays.asList(TableTestSuite.TABLE_ROWS[0]));
        db.table("table").rows()
            .set(1, Arrays.asList(TableTestSuite.TABLE_ROWS[2]));
        db.table("refs").rows()
            .set(1, Arrays.asList(TableTestSuite.REF_ROWS[2]));
        db.table("table3").rows()
            .set(0, Arrays.asList(TableTestSuite.TABLE3_ROWS[3]));
    }
     ...
     * }
     * };
     * Value[][] rows = MyTableTestSuites.createValues(rawData);
     * 
     * MyDatabase db = new MyDatabase();
     * db.createTable("table", MyTableTestSuites.FIELDS_1);
     ...
     * 
     * Value[][] nrows = MyTableTestSuites.createValues(new Object[][] {
     * {
     * 1, "Blah"
     * }
     * });
     * 
     * assertTrue(db.table("table").rows().size() == nrows.length);
     * 
     * for (Value[] row : nrows) {
     * assertTrue(db.table("table").row(row[0]).size() == row.length);
     * for (int i = 0; i != row.length; ++i)
     * assertTrue(db.table("table").row(row[0]).get(i).equals(row[i]));
     * }
     * }
     * 
     * @Test
