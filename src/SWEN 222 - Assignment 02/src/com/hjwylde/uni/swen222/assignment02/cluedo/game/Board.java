...
public final class Board {
    ...
    public Board(Tile[][] board) {
        ...
        Table<Integer, Integer, Tile> table = HashBasedTable.create(
    ...
    public synchronized void addPlayer(Player player) {
        ...
        Optional<Coordinate> pos = getCharacterInitialPosition(player
            .getCharacter());
        checkArgument(pos.isPresent(),
    ...
    public synchronized ImmutableList<Coordinate> getPlayerHistory(int player) {
        checkElementIndex(player, players.size(),
            "Player is not present on the board!");
    ...
    public boolean isEmptyTile(Coordinate pos) {
        ...
        default:
            throw new InternalError(
                "isEmptyTile(Coordinate) not fully implemented: "
                    + getTile(pos));
        }
    ...
    public boolean isValidCoordinate(Coordinate pos) {
        return board.containsRow(pos.getX())
            && board.containsColumn(pos.getY());
    }
    ...
    public void moveWeapon(Weapon weapon, Room room) {
        checkArgument(room != Room.POOL,
            "The weapon cannot be moved to the pool room!");
        
        weaponLocations.put(Objects.requireNonNull(weapon),
