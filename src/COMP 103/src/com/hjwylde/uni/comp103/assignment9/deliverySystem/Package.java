...
public class Package implements Comparable<Package> {
    ...
    public Package(String destination, String pkgType, int dropOffTime) {
        ...
        if (!pkgType.matches("[PSB]"))
            throw new IllegalArgumentException(
                "pkgType must match the regex [PSB].");
    ...
    /**
     * Used for priority queues. A package has a greater priority based on the
     * minimum leaving time in
     * order to deliver that package on time.
     */
    @Override
    public int compareTo(Package pkg) {
        return (int) ((deliveryDue - getDeliveryTime()) - (pkg.deliveryDue - pkg
            .getDeliveryTime()));
    }
    ...
    /**
     * Validates this package in terms of package specifications. A package is a
     * valid package if the
     * current time is less than specified "accept" times for various package
     * types. Note: only needs
     * to be checked once when the package has been dropped off at the depot.
     */
