 ...
 * ISBN : 1-5927-3005-1
 * 
 * This book was a reference for some parts of this assignment, including
 * "scan converting". The term is taken from the book, and the 'base' code
 * format for the ScanConverter, but was re-written for re-use and changed to use depth values. I
 * read this book and wrote similar code (but my own) a year / two ago, so some formats will follow
 * the same methods as the book, but it was written from scratch.
 */
...
import com.hjwylde.uni.comp261.assignment4.graphics.polytopes.Polytope2d;
 ...
 * 
 * @see com.hjwylde.uni.comp261.assignment4.graphics.ScanConverter.Scan
...
public class ScanConverter {
     ...
     * this
     * scan converter has and sets the top and bottom boundaries to the min / max y value of the
     ...
     * the
     * boundary the result will have a list of scans with correct left and right values.
     * 
     * @param face the face to scan convert.
     * @return true if the polygon is visible after scan converted.
     * 
     * @see com.hjwylde.uni.comp261.assignment4.graphics.polytopes.Polytope2d
     */
    public boolean convert(Polytope2d face) {
        ...
        for (int i = 0; i < vertices.size(); i++) {
            ...
            Line3d edge = new Line3d(v1, face.getShade(v1, lights), v2,
                face.getShade(v2, lights)); // Line equation with easy "get" methods.
            ...
            if ((v2.x - v1.x) == 0.0) { // If Vertical line...
                int x = (int) Math
                    .ceil(Math.min(maxX + 1, Math.max(v1.x, minX))); // Calculate x.
                
                for (int y = startY; y <= endY; y++)
                    // For each y in this line...
                    scans[y].setBoundary(x, edge.getWithY(y).z,
                        edge.getShadeWithY(y)); // ... set boundary to
                                                // be x with a depth
                                                // calculated with y.
            } else { // ... else ....
                ...
                if (startX < minX) { // If edge is clipped on left side of screen...
                    int y = (int) Math.ceil(Math.min(edge.getWithX(minX).x,
                        endY)); // Calculate edge y.
                    ...
                    while (startY <= y) { // For each y where x is clipped...
                        ...
                        startY++; // Update new start y.
                                            ...
                                            // screen...
                    int y = (int) Math.ceil(Math.min(edge.getWithX(maxX).y,
                        endY)); // Calculate edge y.
                    
                    while (startY <= y) { // For each y where x is clipped...
                        scans[startY].setBoundary(maxX + 1,
                            edge.getWithX(maxX + 1).z,
                ...
                if (endX < minX) { // If edge is clipped on left side of screen...
                    int y = (int) Math.ceil(Math.max(edge.getWithX(minX).x,
                        startY)); // Calculate edge y.
                    ...
                    int y = (int) Math.ceil(Math.max(edge.getWithX(maxX).x,
                        startY)); // Calculate edge y.
                ...
                for (int y = startY; y <= endY; y++)
                    ...
                    scans[y].setBoundary((int) Math.ceil(edge.getWithY(y).x),
                        edge.getWithY(y).z, edge.getShadeWithY(y)); // Use line equation to
                                                                    // calculate x, depth
                                                                    // and shade at each y.
            }
        }
     ...
     * y
     * value for the polygon that this <code>ScanConverter</code> has converted.
     * 
     ...
     * depth
     * and shade values for these points. Used in conjunction with scan converting and rendering a
     * 3d
     * image with a <code>ZBuffer</code>.
     * 
    ...
    public class Scan {
         ...
         * the
         * depths to the highest possible depth.
         */
        public void clear() {
         ...
         * current
         * right, the right will be set to x. Also sets the depth and shade values if x gets
         * assigned.
         * 
         * @param x the x value.
