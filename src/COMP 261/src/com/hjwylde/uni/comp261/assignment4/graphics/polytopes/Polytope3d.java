...
public class Polytope3d implements Polytope, Iterable<Polytope2d> {
    ...
    public void project(ViewPlane view) {
        for (int i = faces.size() - 1; i >= 0; i--)
            if ((World3d.hiddenRemovalMode ? faces.get(i).facing(
                new Vector3d(0.0, 0.0, 0.0)) : true) // If ((if (hiddenRemovalMode)
                                                     // {isFacing(camera)}) &&
                                                     // isValid()...
                && faces.get(i).valid())
     ...
     * of
     * "Vertex -> Normal". It then will update the normal for each vertex as it iterates through
     * every
     * face and every faces vertices.
     */
    private void calcVertexNormals() {
        ...
        for (Polytope2d face : faces) { // For each face...
            ...
            for (Vector3d v : face) { // For each vertex in the current face...
                 ...
                 * the
                 * normal gets updated from consecutive faces, this normal will also be updated.
                 */
