package com.hjwylde.uni.comp261.assignment3.robotSimulator.parser;

import com.hjwylde.uni.comp261.assignment3.robotSimulator.parser.nodes.*;
import com.hjwylde.uni.comp261.assignment3.robotSimulator.parser.types.*;
import com.hjwylde.uni.comp261.assignment3.robotSimulator.parser.types.BooleanConditionType.BooleanConditionOpType;
...
public class Parser {
     ...
     * @throws ParseException if the input ParserScanner does not match the
     *             <i>PROGRAM</i> grammar.
     */
    public static ProgramNode parseProgram(ParserScanner s)
     ...
     * @throws ParseException if the ParserScanner's next values does not match
     *             the
     *             <i>BOOLEAN</i> grammar.
     */
    private static BooleanNode parseBoolean(ParserScanner s)
        throws ParseException {
        BooleanNode bool;
        ...
        case 't': // BooleanType.TRUE | BooleanType.TOUCHING
            ...
            default: // Unknown BooleanType
                throw new ParseException(
                    "Unknown begin characters for a boolean \"t" + s.peek()
                        + "\".");
            }
        ...
        case '!': // BooleanType.NOT
            ...
            break;
        default: // Unknown BooleanType
            throw new ParseException("Unknown begin character for a boolean \""
                + s.peek() + "\".");
        }
     ...
     * @throws ParseException if the ParserScanner's next values does not match
     *             the <i>BOOLEAN_CONDITION</i> grammar.
     */
    private static BooleanConditionNode parseBooleanCondition(ParserScanner s)
        ...
        case '!': // BooleanType.NOT
            ...
            boolCondition = new BooleanConditionNode(
                BooleanConditionType.BOOLEAN);
            ...
            default: // Unknown BooleanConditionOpType
                throw new ParseException("Unknown boolean operator \""
                    + s.peek() + "\".");
            }
     ...
     */
    private static BooleanMethodNode parseBooleanMethod(ParserScanner s)
        throws ParseException {
        s.expectToken();
        switch (s.peek()) { // Look ahead 1.
        ...
        default: // Unknown BooleanMethodType
            throw new ParseException(
                "Unknown begin character for a boolean method \"" + s.peek()
                    + "\".");
        }
    }
     ...
     */
    private static InstructionNode parseInstruction(ParserScanner s)
        throws ParseException {
        ...
        case 'd': // InstructionType.DROP | InstructionType.DO
            ...
            default: // Unknown InstructionType
                throw new ParseException(
                    "Unknown begin characters for an instruction \"d"
                        + s.peek() + "\".");
            }
        ...
        case '$': // InstructionType.SET
            ...
            default:
                throw new ParseException(
                    "Unknown begin character for an operation \"" + s.peek()
                        + "\".");
            }
        ...
        case 't': // InstructionType.TURN | InstructionType.TURN_TOWARDS_FIRST
            ...
            case 'T': // InstructionType.TURN_TOWARDS_FIRST
                instruction = new InstructionNode(
                    InstructionType.TURN_TOWARDS_FIRST);
                
                s.expect("TowardsFirst", false); // Don't clear whitespace before or
                                                 // after expecting.
                instruction.setInstrMethod(Parser.parseInstructionMethod(s)); // Get the next
                // INSTRUCTION_METHOD.
                s.expect(";");
        ...
        default: // Unknown InstructionType
            throw new ParseException(
                "Unknown begin character for an instruction \"" + s.peek()
                    + "\".");
        }
     ...
     * @throws ParseException if the ParserScanner's next values does not match
     *             the <i>INSTRUCTION_METHOD</i> grammar.
     */
    private static InstructionMethodNode parseInstructionMethod(ParserScanner s)
        ...
        default: // Unknown InstructionMethodType
            throw new ParseException(
                "Unknown begin character for an instruction method \""
                    + s.peek() + "\".");
        }
    }
    ...
    private static ValueNode parseValue(ParserScanner s) throws ParseException {
        ...
        if (s.hasNextInt()) { // ValueType.INTEGER
            ...
            default:
                throw new ParseException(
                    "Unknown begin character for a value \"" + s.peek() + "\".");
            }
     ...
     */
    private static ValueMethodNode parseValueMethod(ParserScanner s)
        throws ParseException {
        s.expectToken();
        switch (s.peek()) { // Look ahead 1.
        ...
        case 'N': // ValueMethodType.NUMBER_OF_THINGS_NOT_IN_BOXES
            ...
            return new ValueMethodNode(
                ValueMethodType.NUMBER_OF_THINGS_NOT_IN_BOXES);
        ...
        default:
            throw new ParseException(
                "Unknown begin character for a value method \"" + s.peek()
                    + "\".");
        }
    }
     ...
     * @throws ParseException if the ParserScanner's next values does not match
     *             the
     *             <i>VARIABLE</i> grammar.
     */
    private static VariableNode parseVariable(ParserScanner s)
        throws ParseException {
        s.expect("$", true, false); // Don't clear whitespace after expecting.
        String varName = "$";
        
        s.expectToken(); // Variable name must begin with [a-z].
        while (Character.isDigit(s.peek()) || Character.isLetter(s.peek())
            || (s.peek() == '_')) {
            s.expectToken();
            varName += s.next();
        }
