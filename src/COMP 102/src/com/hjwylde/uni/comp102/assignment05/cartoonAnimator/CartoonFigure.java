...
/**
 * A CartoonFigure object is a cartoon figure, displayed on the screen that can
 * move around, change the direction the figure is facing, change its emotion
 * (smiling or frowning) speak a phrase
 */
...
public class CartoonFigure {
     ...
     * the name of the set of the images, and the coordinates (left, top) of where
     * it should be placed
     */
    ...
    public void talk(String words) {
        ...
        canvas.drawString(words, bubbleX + 12,
            bubbleY + (bubbleHeight / 2) + 3, false);
        canvas.display();
        try {
            Thread.sleep(1000);
        } catch (Exception e) {}
        
        canvas.clearRect(bubbleX, bubbleY, bubbleWidth + 1, bubbleHeight + 1,
            false);
    }
    ...
    /**
     * Helper method that draws the CartoonFigure All the public methods that
     * change the figure call draw.
     */
    ...
    private void draw() {
        ...
        canvas.drawImage(filename, figureX, figureY, figureWidth, figureHeight,
            false);
        canvas.display();
        try {
            Thread.sleep(500);
    ...
    /**
     * Helper method that erases the CartoonFigure All the public methods that
     * change the figure call erase first
     */
    
    private void erase() {
        canvas.clearRect(figureX, figureY, figureWidth + 1, figureHeight + 1,
            false);
    }
