...
/**
 * Displays an animated cartoon of two characters from a script in a file. The
 * first two lines of the file are the names (imagefile names) and initial
 * positions of the two characters. Each remainin line in the file contains the
 * name of a character, followed by a command. If the command is "move", it is
 * followed by a distance to move If the command is "say", it is followed by the
 * text to say.
 */
public class CartoonAnimator {
    ...
    /**
     * animateFromFile first creates a JFrame window and a DrawingCanvas It then
     * opens a script file, and reads the names and intial positions of the two
     * characters from the first two lines of the file. It then creates two
     * CartoonFigure objects, using the names as the imageNames and places them on
     * the window. It then has a loop to read each instruction from the script
     * file: Each instruction starts with the name of the character, followed by a
     * command: left, right, move, smile, frown, or talk If the command is move,
     * it must be followed by a single integer (the distance to move). If the
     * command is talk, it must be followed by some words to say. It must call the
     * command on the correct CartoonFigure object. The loop will exit when there
    ...
    private void createFigure(String line) {
        ...
        NamedFigure namedFigure = new NamedFigure(figureInitiator.nextToken(),
            new CartoonFigure(canvas, figureInitiator.nextToken(),
   ...
   */
    private void readLine(StringTokenizer line) {
        ...
        if (code.equals("green") || code.equals("blue")
            || code.equals("yellow")) {
            createFigure(name + " " + code + " " + line.nextToken() + " "
                + line.nextToken());
            return;
        }
        ...
        if (currentFigure == null) {
            System.err.println("Cannot use figure: " + name
        ...
        if (code.equals("enter"))
            ...
            System.err.println("Cannot use figure: " + name
                + ". It has not yet entered the world.");
            return;
        }
    ...
    /**
     * The NamedFigure class represents a CartoonFigure with a name, allowing them
     * to be searched for by a specific name.
     */
